export const WORLD_REGIONS = Object.freeze([
  ...
  { id: "north-central-america", label: "北・中央アメリカ", west: -171, east: -74, north: 76, south: 4 },
...
export function geoBoundsToViewBox({ west, east, north, south }) {
  const width = 2000;
  const height = 857;
  const x = ((west + 180) / 360) * width;
  const y = ((90 - north) / 180) * height;
  const w = ((east - west) / 360) * width;
  const h = ((north - south) / 180) * height;
  return [x, y, w, h].map((value) => Math.round(value * 10) / 10).join(" ");
}
