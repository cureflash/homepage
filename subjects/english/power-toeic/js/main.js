...
function saveProgressionEvent(event) { const next = applyProgressionEvent(currentProgression(), event); appStore.replaceProgression({ points: next.points, stage: next.stage }); return next; }
function showEditor(recipe = activeRecipe ?? defaultRecipe) { activeRecipe = recipe; questionReporter.close(); hideViews(); editorView.hidden = false; progressEl.textContent = '設定'; editor.open(recipe); }
function startSession(recipe) { const state = appStore.load(); const reviewQuestionIds = recipe.mode === 'REVIEW' ? getDueReviewQuestionIds(state.reviewEntries) : []; const questionIds = selectQuestionIds({ repository, recipe, attempts: state.attempts, reviewQuestionIds }); if (!questionIds.length) return showHome(); activeRecipe = recipe; sessionCompletionAwarded = false; session = new QuizSession({ questionIds, repository, context: sessionContextForRecipe(recipe) }); hideViews(); quizView.hidden = false; renderCurrent(); }
function renderCurrent() { questionReporter.close(); const question = session.currentQuestion; if (!question) return finishSession(); const { current, total } = session.progress; const stage = currentTraineeStage(); const presentation = getQuestionPresentation({ recipe: activeRecipe, question, skillLabels }); progressEl.textContent = `${current} / ${total}`; contextEl.textContent = presentation.contextText; feedbackEl.textContent = '軍曹「この一問を仕上げろ！」'; traineeStatusEl.textContent = `訓練生ステージ ${stage} ・ ${currentProgression().points} POWER`; characters.render({ traineeStage: stage, reaction: 'neutral' }); nextButton.hidden = true; renderer.render(question); playQuestionSound(); }
document.querySelector('[data-action="home"]').addEventListener('click', showHome);
reportButton.addEventListener('click', () => { if (session?.currentQuestion) questionReporter.open(session.currentQuestion); });
renderer.setAnswerHandler((selectedIndex) => { const question = session.currentQuestion; const beforeState = appStore.load(); const attempt = session.submitAnswer(selectedIndex); const previousReview = beforeState.reviewEntries.find((entry) => entry.questionId === attempt.questionId) ?? null; const nextReview = createReviewEntryFromAttempt(attempt, previousReview); appStore.appendAttempt(attempt); appStore.replaceReviewEntries(upsertReviewEntry(beforeState.reviewEntries, nextReview)); const progression = saveProgressionEvent(progressionEventFromAttempt(attempt, beforeState.attempts)); renderer.showResult({ selectedIndex, correctIndex: question.correctIndex, explanation: question.explanation }); playAnswerSound(attempt.correct); const reaction = attempt.correct ? 'correct' : 'wrong'; characters.render({ traineeStage: progression.stage, reaction }); traineeStatusEl.textContent = progression.earned > 0 ? `+${progression.earned} POWER ・ ステージ ${progression.stage}` : `ステージ ${progression.stage} ・ ${progression.points} POWER`; feedbackEl.textContent = attempt.correct ? '軍曹「よし、その調子だ！」' : '軍曹「違う。理由を確認して次だ！」'; nextButton.textContent = session.progress.current === session.progress.total ? '結果を見る' : '次の問題'; nextButton.hidden = false; });
nextButton.addEventListener('click', () => { session.next(); renderCurrent(); });
resultView.addEventListener('click', (event) => { if (event.target.closest('[data-action="restart"]')) startSession(activeRecipe ?? defaultRecipe); if (event.target.closest('[data-action="edit-workout"]')) showEditor(activeRecipe ?? defaultRecipe); });
function finishSession() { questionReporter.close(); if (!sessionCompletionAwarded) { saveProgressionEvent({ type: 'session_complete', questionCount: session.attempts.length }); sessionCompletionAwarded = true; } characters.render({ traineeStage: currentTraineeStage(), reaction: 'complete' }); hideViews(); resultView.hidden = false; renderResults(resultView, session.getResults(), skillLabels); const controls = resultView.querySelector('.result-actions'); if (controls && !controls.querySelector('[data-action="edit-workout"]')) { const edit = document.createElement('button'); edit.type = 'button'; edit.className = 'secondary-button'; edit.dataset.action = 'edit-workout'; edit.textContent = '内容を調整'; controls.append(edit); } }
showHome();
