...
from scripts.publish_jh3_expansion_basics import SEEDS,SKILLS,PROBLEM_COUNT,gen,ans,publish
...
def main():
    ...
    for skill in SKILLS:
        ...
        for seed in SEEDS:
            ...
            keys=set()
            for p in ps:
                assert tuple(ans(p))==tuple(p['answer']); k=json.dumps(p,sort_keys=True); assert k not in keys; keys.add(k)
    ...
    text=(ROOT/'scripts/publish_jh3_expansion_basics.py').read_text(encoding='utf-8');assert 'colors.red' in text and 'str(i+1)' in text
    print('jh3 expansion basics publisher tests: OK')
