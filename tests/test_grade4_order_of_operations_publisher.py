...
from scripts.publish_grade4_order_of_operations import (
    PATTERNS,
    PROBLEM_COUNT,
    SEEDS,
    SKILL,
...
def main():
    ...
    for seed in SEEDS:
        ...
        for problem in problems:
            ...
            seen.add(key)
            assert independent_answer(problem) >= 0
    ...
    source_text = (ROOT / 'scripts/publish_grade4_order_of_operations.py').read_text(encoding='utf-8')
    assert "c.drawString(x, y, str(index + 1))" in source_text
    assert "colors.red" in source_text
    print('grade 4 order-of-operations publisher tests: OK')
