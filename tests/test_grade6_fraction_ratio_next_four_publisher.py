...
from scripts.publish_grade6_fraction_ratio_next_four import (
    PROBLEM_COUNT,
    SEEDS,
    SKILLS,
    generate,
...
def main():
    ...
    own = {f'e6-{skill}-{variant:02d}' for skill in SKILLS for variant, _ in enumerate(SEEDS, 1)}
        ...
        for seed in SEEDS:
            ...
            for problem in problems:
                ...
                if skill == 'simplify-ratio':
                    left, right = map(int, answer.split(':'))
                    import math
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert all(entry['grade'] == 6 and entry['subject'] == '算数' for entry in catalog)
    ...
    source_text = (ROOT / 'scripts/publish_grade6_fraction_ratio_next_four.py').read_text(encoding='utf-8')
    assert 'str(index + 1)' in source_text
    assert 'colors.red' in source_text
    print('grade 6 fraction/ratio next-four publisher tests: OK')
