...
from scripts.publish_grade6_ratio_algebra_speed_four import (
    PROBLEM_COUNT,
    SEEDS,
    SKILLS,
    generate,
...
def main():
        ...
        for seed in SEEDS:
            ...
            for problem in problems:
                ...
                if skill == 'ratio-value':
                    assert answer.numerator * problem['b'] == answer.denominator * problem['a']
                elif skill == 'proportion-missing-value':
                    assert problem['a'] * answer == problem['b'] * problem['c']
                    ...
                    if problem['target'] == 'distance':
                        assert answer == problem['speed'] * problem['time']
                    elif problem['target'] == 'speed':
                        assert answer * problem['time'] == problem['distance']
                    else:
                        assert answer * problem['speed'] == problem['distance']
    ...
    source_text = (ROOT / 'scripts/publish_grade6_ratio_algebra_speed_four.py').read_text(encoding='utf-8')
    assert 'str(index + 1)' in source_text
    assert 'colors.red' in source_text
    print('grade 6 ratio/algebra/speed publisher tests: OK')
