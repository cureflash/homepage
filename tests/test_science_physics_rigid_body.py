...
class PhysicsRigidBodyTests(unittest.TestCase):
        ...
        for topic_key, mode_key, variant, seed, problems in self.generated_batches():
            for problem in problems:
                ...
                if topic_key == "rigid-body-force-moment":
                    if solve_for == "moment":
                        expected = known["force"] * known["lever_arm"]
                    elif solve_for == "force":
                        expected = known["moment"] / known["lever_arm"]
                    else:
                        expected = known["moment"] / known["force"]
                elif topic_key == "rigid-body-weight-moment":
                    if solve_for == "moment":
                        expected = known["mass"] * known["gravity"] * known["lever_arm"]
                    elif solve_for == "mass":
                        expected = known["moment"] / (known["gravity"] * known["lever_arm"])
                    else:
                        expected = known["moment"] / (known["mass"] * known["gravity"])
                elif topic_key == "rigid-body-couple-moment":
                    if solve_for == "couple_moment":
                        expected = known["force"] * known["line_separation"]
                    elif solve_for == "force":
                        expected = known["couple_moment"] / known["line_separation"]
                    else:
                        expected = known["couple_moment"] / known["force"]
                else:
                    self.fail(topic_key)
    ...
    def test_scope_units_and_explicit_geometry(self):
