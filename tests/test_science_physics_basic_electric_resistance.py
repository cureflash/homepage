...
from scripts.science_physics_basic_electric_resistance import (
    PHYSICS_BASIC_ELECTRIC_RESISTANCE_PROBLEM_COUNT,
...
def generated_sets():
    for topic_key, topic in PHYSICS_BASIC_ELECTRIC_RESISTANCE_TOPICS.items():
...
def test_exactly_forty_variants_and_four_modes():
    ...
    assert all(len(problems) == PHYSICS_BASIC_ELECTRIC_RESISTANCE_PROBLEM_COUNT for *_, problems in batches)


def test_deterministic_independent_validation_and_unique_hashes():
    hashes = set()
    for topic_key, mode_key, seed, problems in generated_sets():
        mode = PHYSICS_BASIC_ELECTRIC_RESISTANCE_TOPICS[topic_key]["modes"][mode_key]
        again = generate_retrieval_drill(mode["spec"], seed, PHYSICS_BASIC_ELECTRIC_RESISTANCE_PROBLEM_COUNT, mode=mode_key)
...
def test_independent_validation_rejects_corruption():
    mode = PHYSICS_BASIC_ELECTRIC_RESISTANCE_TOPICS["electric-resistance-concepts"]["modes"]["fill"]
    problem = generate_retrieval_drill(mode["spec"], 11001, 1, mode="fill")[0]
    ...
    try:
        ...
        raise AssertionError("corrupted electric-resistance retrieval answer was accepted")


def test_curriculum_concepts_are_present():
    topic = PHYSICS_BASIC_ELECTRIC_RESISTANCE_TOPICS["electric-resistance-concepts"]
    pairs = topic["modes"]["forward"]["spec"]["items"]
    lefts = {item["left"] for item in pairs}
    assert "抵抗率" in lefts
    assert "金属中の電流の担い手" in lefts
    assert "同じ物質・同じ断面積で導体を長くしたときの抵抗" in lefts
    assert "同じ物質・同じ長さで導体の断面積を大きくしたときの抵抗" in lefts


def test_hashes_are_disjoint_from_resonance_retrieval():
    ...
    old_hashes = set()
    for topic in PHYSICS_BASIC_RESONANCE_TOPICS.values():
        for mode_key, mode in topic["modes"].items():
            for seed in topic["seeds"]:
...
if __name__ == "__main__":
    ...
    test_hashes_are_disjoint_from_resonance_retrieval()
    print("Physics Basics electric resistance retrieval tests: OK")
