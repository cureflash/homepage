...
const batchUrl = new URL('../data/grade2-authoring-color-management-profiles-0001-0012.json', import.meta.url);
...
if (existingSkill) {
  ...
  if (JSON.stringify(existingSkill) !== JSON.stringify(batch.skill) || JSON.stringify(existingQuestions) !== JSON.stringify(batch.questions)) {
    throw new Error('pc2.media.color_management_profiles exists in runtime but is not record-identical to verified authoring');
  }
  process.exit(0);
}
