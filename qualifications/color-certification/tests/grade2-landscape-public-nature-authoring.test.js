...
const batch = JSON.parse(await readFile(new URL('../data/grade2-authoring-landscape-public-nature-0001-0012.json', import.meta.url), 'utf8'));
...
test('Grade 2 Landscape p116 public-nature batch is independently verified, nonvisual, balanced and runtime-promoted record-identically', () => {
  assert.equal(batch.grade, 2);
  assert.equal(batch.skill.id, 'pc2.landscape.public_nature');
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade2/landscape/public_nature');
    ...
    assert.equal(q.sourceRefs.includes('mlit_landscape_act_basic_principles_2026'), true);
