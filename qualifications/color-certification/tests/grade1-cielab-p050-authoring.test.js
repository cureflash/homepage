...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-cielab-p050-0001-0004.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p050 CIELAB batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.color_representation.cielab_p050');
  ...
  const allowedSources = new Set(['cie_cielab_17_23_076','cie_cielab_difference_17_23_077','iso_cie_11664_4_2019']);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath,'grade1/color_representation/cielab_p050');
