...
const fileName = 'grade1-authoring-landscape-color-standard-role-p142-0001-0004.json';
...
test('Grade 1 p142 color-standard-role batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade,1); assert.equal(batch.skill.id,'pc1.landscape.color_standard_role_p142'); assert.equal(batch.questions.length,4);
