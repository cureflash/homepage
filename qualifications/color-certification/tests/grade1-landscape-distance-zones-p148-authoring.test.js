...
const fileName = 'grade1-authoring-landscape-distance-zones-p148-0001-0004.json';
...
test('Grade 1 p148 distance-zones batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade,1); assert.equal(batch.skill.id,'pc1.landscape.distance_zones_p148'); assert.equal(batch.questions.length,4);
