...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-measurement-illuminants-p039-0001-0004.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p039 measurement-illuminants batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.light_color.measurement_illuminants_p039');
  ...
  const allowedSources = new Set(['cie_illuminant_17_23_018','cie_standard_illuminant_d65_17_23_021','cie_iso11664_2_2022']);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath,'grade1/light_color/measurement_illuminants_p039');
