...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-light-color-appearance-p032-0001-0003.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p032 appearance-mode batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.light_color.appearance_mode_p032');
  assert.equal(batch.questions.length, 3);
  assert.deepEqual(batch.qaSummary, {generated:3,checked:3,verified:3,needsRevision:0,rejected:0,pending:0,correctIndexDistribution:{A:1,B:1,C:1,D:0}});
  ...
  const allowedSources = new Set(['pubmed_kuriki_2015','cinii_iieej_2007','pubmed_yamauchi_uchikawa_2005']);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath,'grade1/light_color/appearance_mode_p032');
