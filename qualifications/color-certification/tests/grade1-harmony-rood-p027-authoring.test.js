...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-harmony-rood-p027-0001-0004.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p027 Rood batch remains verified, source-bound, nonvisual and globally unique', () => {
  ...
  const allowedSources = new Set(['loc_rood_modern_chromatics', 'met_rood_optical_mixture', 'iscc_rood_summary']);
  ...
  for (const q of batch.questions) {
    ...
    assert.ok(q.sourceRefs.length >= 1 && q.sourceRefs.every((id) => allowedSources.has(id)));
