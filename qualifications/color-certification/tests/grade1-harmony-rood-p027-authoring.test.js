...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-harmony-rood-p027-0001-0003.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p027 Rood batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.harmony.rood_p027');
  assert.equal(batch.questions.length, 3);
  assert.deepEqual(batch.qaSummary, {
    generated: 3, checked: 3, verified: 3, needsRevision: 0, rejected: 0, pending: 0,
    correctIndexDistribution: { A: 1, B: 1, C: 1, D: 0 }
  });
  ...
  const allowedSources = new Set(['met_rood_modern_chromatics', 'uchicago_rood_optical_mixture', 'met_rood_complementary_context']);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade1/harmony/rood_p027');
    ...
    assert.equal(q.sourceRefs.length, 1);
    assert.equal(allowedSources.has(q.sourceRefs[0]), true);
  ...
  assert.deepEqual(positions, [1, 1, 1, 0]);
});
