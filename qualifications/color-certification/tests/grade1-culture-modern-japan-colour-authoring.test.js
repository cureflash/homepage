...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-culture-modern-japan-colour-0001-0008.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p021 modern Japan culture batch remains verified, source-bound, nonvisual, globally unique and balanced', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.culture.modern_japan_colour');
  ...
  const allowedSources = new Set([
    'kyoto_national_museum_yuzen_chemical_dyes',
    'agency_cultural_affairs_tenjin_zu_aniline',
    'ndl_wada_shikimei_sokan_1931',
    'ndl_wada_haishoku_sokan_1930s'
  ]);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade1/culture/modern_japan_colour');
