...
const fileName = 'grade1-authoring-landscape-color-standard-operation-p143-0001-0004.json';
...
test('Grade 1 p143 color-standard-operation batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade,1); assert.equal(batch.skill.id,'pc1.landscape.color_standard_operation_p143'); assert.equal(batch.questions.length,4);
