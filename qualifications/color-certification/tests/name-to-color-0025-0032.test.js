...
const previous = JSON.parse(await readFile(new URL('../data/grade3-authoring-name-to-color-0017-0024.json', import.meta.url), 'utf8'));
const authoring = JSON.parse(await readFile(new URL('../data/grade3-authoring-name-to-color-0025-0032.json', import.meta.url), 'utf8'));

const colorById = new Map(colors.colors.map((color) => [color.id, color]));

test('name-to-color checkpoint 0025-0032 is independently verified and internally consistent', () => {
  ...
  assert.equal(authoring.questions[0].id, 'pc3-conventional-name-to-color-0025');
  assert.equal(authoring.questions.at(-1).id, 'pc3-conventional-name-to-color-0032');
...
test('name-to-color staged coverage reaches 24 unique master refs without target reuse', () => {
  ...
  const stagedTargets = [...previous.questions, ...authoring.questions].map((question) => question.colorRef);
  const allTargets = [...runtimeTargets, ...stagedTargets];
  assert.equal(runtimeTargets.length, 8);
  assert.equal(stagedTargets.length, 16);
