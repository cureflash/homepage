...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-ncs-hue-circle-notation-p056-0001-0004.json', import.meta.url), 'utf8'));
const priorFiles = [
  ...
  '../data/grade1-authoring-ncs-hering-six-colors-p056-0001-0004.json'
];
...
test('Grade 1 p056 NCS hue-circle/notation batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.color_representation.ncs_hue_circle_notation_p056');
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade1/color_representation/ncs_hue_circle_notation_p056');
