...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-lighting-properties-p036-0001-0004.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p036 lighting-properties batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.light_color.lighting_properties_p036');
  ...
  const allowedSources = new Set(['cie_luminous_flux_17_21_039','cie_lumen_17_21_084','cie_luminous_intensity_17_21_045','cie_lux_17_21_085','cie_colour_temperature_17_23_067']);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath,'grade1/light_color/lighting_properties_p036');
