...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-grassmann-laws-p041-0001-0004.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p041 Grassmann-laws batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.light_color.grassmann_laws_p041');
  ...
  const allowedSources = new Set(['cie_grassmann_17_23_033','cie_colour_equation_17_23_040']);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath,'grade1/light_color/grassmann_laws_p041');
