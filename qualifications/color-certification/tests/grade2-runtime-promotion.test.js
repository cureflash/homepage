...
test('Grade 2 runtime is the record-identical union of verified authoring batches', () => {
  ...
  assert.equal(runtime.questions.length, 252);
  assert.equal(runtime.questions.filter((q) => q.validationStatus === 'verified').length, 252);
...
test('shared Power TOEIC engine runs all Grade 2 skills', () => {
  ...
  assert.equal(selectQuestionIds({ repository, recipe }).length, 63);
});
