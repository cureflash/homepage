...
test('Grade 2 runtime is the record-identical union of verified authoring batches', () => {
  ...
  assert.equal(runtime.questions.length, 264);
  assert.equal(runtime.questions.filter((q) => q.validationStatus === 'verified').length, 264);
...
test('shared Power TOEIC engine runs all Grade 2 skills', () => {
  ...
  assert.equal(selectQuestionIds({ repository, recipe }).length, 66);
});
