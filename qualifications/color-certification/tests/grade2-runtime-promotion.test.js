...
const batches = [foundation, triad, munsell, naturalComplex, dominant, toneOnTone, toneInTone, tonal, camaieu, bicolorTricolor, hueCircleDivision, imageSchemeKeywords, rgbCmykModels, colorManagementProfiles, imageBasics, basicTrend, interiorColorPlanning, interiorZoning, interiorElementColorRoles, interiorStyleNames, landscapePublicNature, landscapeElementExamples, landscapeColorDesignHarmonyRole, landscapeRegionalColorContext, landscapeLongLifecycleColor, landscapeDesignConsiderations, landscapeResidentialColorDesignProcess];
...
test('Grade 2 runtime is the record-identical union of verified authoring batches', () => {
  ...
  assert.equal(runtime.questions.length, 324);
  assert.equal(runtime.questions.filter((q) => q.validationStatus === 'verified').length, 324);
...
test('shared Power TOEIC engine runs all Grade 2 skills', () => {
  ...
  assert.equal(selectQuestionIds({ repository, recipe }).length, 81);
});
