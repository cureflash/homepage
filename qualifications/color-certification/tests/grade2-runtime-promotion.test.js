...
test('Grade 2 runtime is the record-identical union of verified authoring batches', () => {
  ...
  assert.equal(runtime.questions.length, 24);
  assert.equal(runtime.questions.filter((q) => q.validationStatus === 'verified').length, 24);
  assert.equal(runtime.questions.filter((q) => q.validationStatus === 'pending_validation').length, 0);
  assert.deepEqual(runtime.questions, [...foundation.questions, ...triad.questions]);
  assert.deepEqual(runtime.skills, [foundation.skill, triad.skill]);
});
...
test('shared Power TOEIC engine runs both Grade 2 skills', () => {
  const repository = new InMemoryQuestionBank({ questions: runtime.questions, skills: runtime.skills });
  for (const targetId of [foundation.questions[0].id, triad.questions[0].id]) {
  ...
  const recipe = createWorkoutRecipe({
    ...
    totalCount: 12,
    skillAllocations: [
      { skillId: 'pc2.foundation.official_sample_facts', count: 6 },
      { skillId: 'pc2.scheme.triad_hue_positions', count: 6 }
    ],
    seed: 41
  });
