...
const batch = JSON.parse(await readFile(new URL('../data/grade2-authoring-landscape-residential-color-design-process-0001-0012.json', import.meta.url), 'utf8'));
...
test('Grade 2 Landscape p123 residential process batch remains independently verified, nonvisual, balanced and record-identical after promotion', () => {
  ...
  const allowedSources = new Set(['aft_grade2_current_toc_2026', 'jpma_ccs_2025_practical']);
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade2/landscape/residential_color_design_process');
