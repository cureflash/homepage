...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-ostwald-color-solid-p055-0001-0003.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p055 Ostwald color-solid batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.color_representation.ostwald_color_solid_p055');
  ...
  const allowedSources = new Set(['granville_ostwald_1994']);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath,'grade1/color_representation/ostwald_color_solid_p055');
