...
const fileName = 'grade1-authoring-landscape-color-standard-presentation-p144-0001-0004.json';
...
test('Grade 1 p144 color-standard-presentation batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade,1); assert.equal(batch.skill.id,'pc1.landscape.color_standard_presentation_p144'); assert.equal(batch.questions.length,4);
  ...
  for(const q of batch.questions){assert.equal(q.validationStatus,'verified');assert.equal(q.qa.generatedAs,'pending_validation');assert.match(q.qa.independentRecheck,/^independently /);assert.equal(q.questionType,'text_choice');assert.equal(q.taxonomyPath,'grade1/landscape/color_standard_presentation_p144');assert.equal(q.prompt,q.sentence);assert.equal(q.choices.length,4);assert.equal(new Set(q.choices).size,4);assert.equal(q.proposedAnswer,q.choices[q.correctIndex]);assert.ok(q.sourceRefs.includes('aft_grade1_toc_current'));assert.ok(q.sourceRefs.includes('tokyo_color_standard_table') || q.sourceRefs.includes('yokosuka_munsell_guidance'));assert.equal('presentation' in q,false);assert.equal('colorRefs' in q,false);assert.equal(ids.has(q.id),false);assert.equal(fps.has(fp(q)),false);ids.add(q.id);fps.add(fp(q));positions[q.correctIndex]+=1;const feedback=getAnswerFeedbackModel(q,new Map());assert.equal(feedback.kind,'text');assert.equal(feedback.showSwatch,false);} assert.deepEqual(positions,[1,1,1,1]);
});
