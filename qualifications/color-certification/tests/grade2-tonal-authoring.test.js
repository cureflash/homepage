...
const batch = JSON.parse(await readFile(new URL('../data/grade2-authoring-tonal-0001-0012.json', import.meta.url), 'utf8'));

function fingerprint(q) {
  return JSON.stringify([q.sentence, q.choices]);
}

test('Grade 2 tonal authoring batch is independently verified and balanced', () => {
  assert.equal(batch.grade, 2);
  assert.equal(batch.skill.id, 'pc2.scheme.tonal');
  ...
  const canonical = new Set(['sf','d','ltg','g']);
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade2/scheme/tonal');
    ...
    assert.ok(q.sourceRefs.includes('pluscolors_tonal_2024'));
    assert.ok(q.sourceRefs.includes('uto_tonal_2023'));
  ...
  assert.deepEqual([...canonical].sort(), ['d','g','ltg','sf'].sort());
});
