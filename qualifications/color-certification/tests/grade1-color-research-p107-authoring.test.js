...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-color-research-p107-0001-0004.json', import.meta.url), 'utf8'));
const priorFiles = [
  '../data/grade1-authoring-color-marketing-basic-view-p104-0001-0004.json'
];
...
test('Grade 1 p107 color-research batch remains verified, AFT-source-bound, nonvisual and unique', () => {
  assert.equal(batch.grade,1); assert.equal(batch.skill.id,'pc1.business.color_research_p107'); assert.equal(batch.questions.length,4);
