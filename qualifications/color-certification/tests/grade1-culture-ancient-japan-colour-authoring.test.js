...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-culture-ancient-japan-colour-0001-0008.json', import.meta.url), 'utf8'));

function fingerprint(q) { return JSON.stringify([q.sentence, q.choices]); }

test('Grade 1 p014 ancient Japan culture batch remains verified, source-bound, nonvisual and balanced', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.culture.ancient_japan_colour');
  ...
  const allowedSources = new Set([
    'imperial_household_shosoin_treasure',
    'kyoto_national_museum_ancient_textiles',
    'kyoto_national_museum_green_nishiki_catalogue'
  ]);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade1/culture/ancient_japan_colour');
