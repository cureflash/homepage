...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-ostwald-color-notation-p054-0001-0004.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p054 Ostwald notation batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.color_representation.ostwald_color_notation_p054');
  ...
  const allowedSources = new Set(['cra_ostwald_2023','ies_ostwald_system','jcsa_ritsumei_2024']);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath,'grade1/color_representation/ostwald_color_notation_p054');
