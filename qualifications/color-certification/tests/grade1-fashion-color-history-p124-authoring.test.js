...
const fileName = 'grade1-authoring-fashion-color-history-p124-0001-0008.json';
...
test('Grade 1 p124 fashion-color-history batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade,1);
  assert.equal(batch.skill.id,'pc1.fashion.color_history_p124');
  assert.equal(batch.questions.length,8);
  assert.deepEqual(batch.qaSummary,{generated:8,checked:8,verified:8,needsRevision:0,rejected:0,pending:0,correctIndexDistribution:{A:2,B:2,C:2,D:2}});
  ...
  for(const q of batch.questions){
    ...
    assert.equal(q.taxonomyPath,'grade1/fashion/color_history_p124');
    assert.equal(q.prompt,q.sentence);
    assert.equal(q.choices.length,4);
    assert.equal(new Set(q.choices).size,4);
    assert.equal(q.proposedAnswer,q.choices[q.correctIndex]);
    assert.ok(q.sourceRefs.includes('aft_grade1_toc_current'));
    assert.ok(q.sourceRefs.includes('bnn_fashion_color100_toc'));
