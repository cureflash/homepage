...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-culture-modern-europe-color-science-0001-0008.json', import.meta.url), 'utf8'));

function fingerprint(q) { return JSON.stringify([q.sentence, q.choices]); }

test('Grade 1 p010 modern Europe color-science batch remains verified, source-bound, nonvisual and balanced', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.culture.modern_europe_color_science');
  ...
  const allowedSources = new Set([
    'royal_society_newton_1672',
    'royal_society_newton_1675',
    'royal_society_newton_1676_opticks',
    'royal_society_newton_goethe_1817',
    'klassik_stiftung_goethe_farbenlehre',
    'klassik_stiftung_goethe_200_years'
  ]);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade1/culture/modern_europe_color_science');
