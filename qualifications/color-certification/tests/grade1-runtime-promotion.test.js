...
const rood = JSON.parse(await readFile(new URL('../data/grade1-authoring-harmony-rood-p027-0001-0003.json', import.meta.url), 'utf8'));
const ostwald = JSON.parse(await readFile(new URL('../data/grade1-authoring-harmony-ostwald-p028-0001-0004.json', import.meta.url), 'utf8'));
const itten = JSON.parse(await readFile(new URL('../data/grade1-authoring-harmony-itten-p029-0001-0004.json', import.meta.url), 'utf8'));
const moonSpencer = JSON.parse(await readFile(new URL('../data/grade1-authoring-harmony-moon-spencer-p030-0001-0004.json', import.meta.url), 'utf8'));
const appearanceMode = JSON.parse(await readFile(new URL('../data/grade1-authoring-light-color-appearance-p032-0001-0003.json', import.meta.url), 'utf8'));
const lightingProperties = JSON.parse(await readFile(new URL('../data/grade1-authoring-lighting-properties-p036-0001-0004.json', import.meta.url), 'utf8'));
const measurementIlluminants = JSON.parse(await readFile(new URL('../data/grade1-authoring-measurement-illuminants-p039-0001-0004.json', import.meta.url), 'utf8'));
...
const cielab = JSON.parse(await readFile(new URL('../data/grade1-authoring-cielab-p050-0001-0004.json', import.meta.url), 'utf8'));
const ostwaldHueCircle = JSON.parse(await readFile(new URL('../data/grade1-authoring-ostwald-hue-circle-p053-0001-0004.json', import.meta.url), 'utf8'));
const ostwaldColorNotation = JSON.parse(await readFile(new URL('../data/grade1-authoring-ostwald-color-notation-p054-0001-0004.json', import.meta.url), 'utf8'));
const ostwaldEqualHuePlane = JSON.parse(await readFile(new URL('../data/grade1-authoring-ostwald-equal-hue-plane-p054-0001-0004.json', import.meta.url), 'utf8'));
const batches = [ancientEurope, medievalEurope, modernEurope, ancientJapan, ancientToEarlyModernJapan, modernJapan, judd, chevreul, rood, ostwald, itten, moonSpencer, appearanceMode, lightingProperties, measurementIlluminants, grassmannLaws, xyz, cielab, ostwaldHueCircle, ostwaldColorNotation, ostwaldEqualHuePlane];
...
test('Grade 1 runtime contains only record-identical verified authoring batches', () => {
  ...
  assert.equal(runtime.questions.length, 105);
  assert.equal(runtime.questions.filter((question) => question.validationStatus === 'verified').length, 105);
...
test('Grade 1 runtime has no full-fingerprint duplicates and keeps answer positions auditable', () => {
  ...
  assert.deepEqual(counts, [27, 27, 27, 24]);
});
