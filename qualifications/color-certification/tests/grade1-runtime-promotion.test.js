...
test('Grade 1 runtime contains only record-identical verified authoring batches', () => {
  ...
  assert.equal(runtime.questions.length, 8);
  assert.equal(runtime.questions.filter((question) => question.validationStatus === 'verified').length, 8);
...
test('shared Power TOEIC engine runs promoted Grade 1 questions', () => {
  ...
  const recipe = createWorkoutRecipe({
    mode: 'TRAINING',
    totalCount: 8,
    skillAllocations: [{ skillId: ancientEurope.skill.id, count: 8 }],
    seed: 43
  });
