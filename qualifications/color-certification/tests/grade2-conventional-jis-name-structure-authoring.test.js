...
const batch = JSON.parse(await readFile(new URL('../data/grade2-authoring-conventional-jis-name-structure-0001-0012.json', import.meta.url), 'utf8'));
...
test('Grade 2 p128 JIS conventional-name structure remains verified, source-bound, nonvisual, balanced and record-identical after promotion', () => {
  assert.equal(batch.grade, 2);
  assert.equal(batch.skill.id, 'pc2.conventional.jis_name_structure');
  ...
  const allowedSources = new Set(['aft_grade2_current_toc_2026', 'jsa_jis_z8102_2001_preview']);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade2/conventional/jis_name_structure');
