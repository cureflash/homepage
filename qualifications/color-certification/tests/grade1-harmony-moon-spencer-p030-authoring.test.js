...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-harmony-moon-spencer-p030-0001-0004.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p030 Moon-Spencer batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.harmony.moon_spencer_p030');
  ...
  const allowedSources = new Set(['jcsa_moon_spencer_2019','ndl_moon_spencer_area','cinii_moon_spencer_geometric','cinii_moon_spencer_aesthetic','jgp_moon_spencer_balance']);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath,'grade1/harmony/moon_spencer_p030');
