...
const authoring = JSON.parse(await readFile(new URL('../data/grade3-authoring-name-to-color-0041-0048.json', import.meta.url), 'utf8'));

const colorById = new Map(colors.colors.map((color) => [color.id, color]));

test('name-to-color checkpoint 0041-0048 is independently verified and internally consistent', () => {
  ...
  assert.equal(authoring.questions[0].id, 'pc3-conventional-name-to-color-0041');
  assert.equal(authoring.questions.at(-1).id, 'pc3-conventional-name-to-color-0048');
...
test('name-to-color staged coverage reaches 40 unique master refs without target reuse', () => {
  ...
  const stagedTargets = [...previous1.questions, ...previous2.questions, ...previous3.questions, ...authoring.questions]
    .map((question) => question.colorRef);
  const allTargets = [...runtimeTargets, ...stagedTargets];
  assert.equal(runtimeTargets.length, 8);
  assert.equal(stagedTargets.length, 32);
  assert.equal(allTargets.length, 40);
  assert.equal(new Set(allTargets).size, 40);
});
