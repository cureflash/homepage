...
const batch = JSON.parse(await readFile(new URL('../data/grade2-authoring-landscape-design-considerations-0001-0012.json', import.meta.url), 'utf8'));
...
test('Grade 2 Landscape p122 considerations batch remains independently verified, nonvisual, balanced and record-identical after promotion', () => {
  assert.equal(batch.grade, 2);
  assert.equal(batch.skill.id, 'pc2.landscape.design_considerations');
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade2/landscape/design_considerations');
    ...
    assert.equal(q.sourceRefs.includes('tokyo_landscape_color_guideline_current'), true);
