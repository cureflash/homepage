...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-harmony-judd-p024-0001-0004.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p024 Judd batch remains verified, source-bound, nonvisual, globally unique and balanced', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.harmony.judd_p024');
  ...
  const allowedSources = new Set(['aft_grade1_toc_p024', 'aft_grade1_sample_harmony', 'waseda_judd_four_principles']);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade1/harmony/judd_p024');
