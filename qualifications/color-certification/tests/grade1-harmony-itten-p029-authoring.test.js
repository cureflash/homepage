...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-harmony-itten-p029-0001-0004.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p029 Itten batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.harmony.itten_p029');
  ...
  const allowedSources = new Set(['getty_itten_color','mit_itten_harmony']);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath,'grade1/harmony/itten_p029');
