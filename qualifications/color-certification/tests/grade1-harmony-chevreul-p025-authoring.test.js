...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-harmony-chevreul-p025-0001-0003.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p025 Chevreul batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.harmony.chevreul_p025');
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade1/harmony/chevreul_p025');
    ...
    assert.deepEqual(q.sourceRefs, ['aft_grade1_sample_chevreul']);
