...
const authoring = JSON.parse(await readFile(new URL('../data/grade3-authoring-name-to-color-0057-0064.json', import.meta.url), 'utf8'));

const colorById = new Map(colors.colors.map((color) => [color.id, color]));

test('name-to-color checkpoint 0057-0064 is independently verified and internally consistent', () => {
  ...
  assert.equal(authoring.questions[0].id, 'pc3-conventional-name-to-color-0057');
  assert.equal(authoring.questions.at(-1).id, 'pc3-conventional-name-to-color-0064');
  assert.equal(new Set(authoring.questions.map((q) => q.id)).size, 8);
  assert.deepEqual(authoring.questions.map((q) => q.colorRef), [
    'c3-conventional-048','c3-conventional-049','c3-conventional-050','c3-conventional-051',
    'c3-conventional-052','c3-conventional-053','c3-conventional-054','c3-conventional-055'
  ]);
...
test('name-to-color staged coverage reaches 56 unique master refs without target reuse', () => {
  ...
  const stagedTargets = [...previous1.questions,...previous2.questions,...previous3.questions,...previous4.questions,...previous5.questions,...authoring.questions].map((q) => q.colorRef);
  const allTargets = [...runtimeTargets, ...stagedTargets];
  assert.equal(runtimeTargets.length, 8);
  assert.equal(stagedTargets.length, 48);
  assert.equal(allTargets.length, 56);
  assert.equal(new Set(allTargets).size, 56);
});
