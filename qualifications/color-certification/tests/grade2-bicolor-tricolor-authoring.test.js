...
const batch = JSON.parse(await readFile(new URL('../data/grade2-authoring-bicolor-tricolor-0001-0012.json', import.meta.url), 'utf8'));

function fingerprint(q) {
  return JSON.stringify([q.sentence, q.choices]);
}

test('Grade 2 bicolor/tricolor authoring batch is independently verified and balanced', () => {
  assert.equal(batch.grade, 2);
  assert.equal(batch.skill.id, 'pc2.scheme.bicolor_tricolor');
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade2/scheme/bicolor_tricolor');
    assert.equal(q.choices.length, 4);
    assert.equal(new Set(q.choices).size, 4);
    assert.equal(q.proposedAnswer, q.choices[q.correctIndex]);
    assert.ok(q.sourceRefs.includes('aft_grade2_toc_2026'));
    assert.ok(q.sourceRefs.includes('pluscolors_bicolor_tricolor_2024'));
    assert.ok(q.sourceRefs.includes('megasoft_bicolor_tricolor'));
