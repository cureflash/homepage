...
const batch = JSON.parse(await readFile(new URL('../data/grade2-authoring-tone-on-tone-0001-0012.json', import.meta.url), 'utf8'));

function fingerprint(q) {
  return JSON.stringify([q.sentence, q.choices]);
}

test('Grade 2 tone-on-tone authoring batch is independently verified and balanced', () => {
  assert.equal(batch.grade, 2);
  assert.equal(batch.skill.id, 'pc2.scheme.tone_on_tone');
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade2/scheme/tone_on_tone');
    ...
    assert.ok(q.sourceRefs.includes('pluscolors_tone_on_tone_2024'));
