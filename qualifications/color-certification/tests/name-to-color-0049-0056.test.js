...
const authoring = JSON.parse(await readFile(new URL('../data/grade3-authoring-name-to-color-0049-0056.json', import.meta.url), 'utf8'));

const colorById = new Map(colors.colors.map((color) => [color.id, color]));

test('name-to-color checkpoint 0049-0056 is independently verified and internally consistent', () => {
  ...
  assert.equal(authoring.questions[0].id, 'pc3-conventional-name-to-color-0049');
...
test('name-to-color staged coverage reaches 48 unique master refs without target reuse', () => {
  ...
  assert.equal(stagedTargets.length, 40);
  assert.equal(allTargets.length, 48);
  assert.equal(new Set(allTargets).size, 48);
});
