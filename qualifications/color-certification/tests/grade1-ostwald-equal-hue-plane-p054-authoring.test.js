...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-ostwald-equal-hue-plane-p054-0001-0004.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p054 Ostwald equal-hue-plane batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.color_representation.ostwald_equal_hue_plane_p054');
  ...
  const allowedSources = new Set(['cra_ostwald_2023','chemtexts_ostwald_2020']);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath,'grade1/color_representation/ostwald_equal_hue_plane_p054');
