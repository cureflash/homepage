...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-xyz-p046-0001-0004.json', import.meta.url), 'utf8'));
const priorFiles = [
...
test('Grade 1 p046 XYZ batch remains verified, source-bound, nonvisual and globally unique', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.color_representation.xyz_p046');
  ...
  const allowedSources = new Set(['cie_xyz_17_23_045','cie_tristimulus_17_23_038']);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath,'grade1/color_representation/xyz_p046');
