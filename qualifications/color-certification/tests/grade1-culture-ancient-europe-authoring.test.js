...
const batch = JSON.parse(await readFile(new URL('../data/grade1-authoring-culture-ancient-europe-materials-0001-0008.json', import.meta.url), 'utf8'));

function fingerprint(q) { return JSON.stringify([q.sentence, q.choices]); }

test('Grade 1 p006 ancient-Europe culture batch remains verified, source-bound, nonvisual and balanced', () => {
  assert.equal(batch.grade, 1);
  assert.equal(batch.skill.id, 'pc1.culture.ancient_europe_materials');
  ...
  const allowedSources = new Set([
    'met_egyptian_blue_lump',
    'met_egyptian_faience_technology',
    'british_museum_parthenon_paint'
  ]);
  ...
  for (const q of batch.questions) {
    ...
    assert.equal(q.taxonomyPath, 'grade1/culture/ancient_europe_materials');
