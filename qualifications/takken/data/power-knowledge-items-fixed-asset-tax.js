(() => {
  ...
  const osakaResidentialLand = Object.freeze({ id: "osaka-city-fixed-asset-residential-land", label: "大阪市『住宅用地の課税標準の特例措置』", url: "https://www.city.osaka.lg.jp/zaisei/page/0000239753.html", sourceType: "official_guidance" });
  const osakaHouse = Object.freeze({ id: "osaka-city-fixed-asset-house", label: "大阪市『家屋関係』", url: "https://www.city.osaka.lg.jp/zaisei/page/0000240554.html", sourceType: "official_guidance" });
  ...
  const knowledgeItems = [
    ...
    { ...shared, knowledgeId: "takken-k-fixed-asset-tax-small-residential-land-one-sixth", claim: "住宅1戸につき200平方メートルまでの小規模住宅用地は、固定資産税の課税標準が価格の6分の1となる。", conditions: ["小規模住宅用地であること"], exceptions: [], importance: "A", primarySources: [source, osakaResidentialLand], sourceFactIds: [] },
    { ...shared, knowledgeId: "takken-k-fixed-asset-tax-general-residential-land-one-third", claim: "小規模住宅用地以外の一般住宅用地は、固定資産税の課税標準が価格の3分の1となる。", conditions: ["一般住宅用地であること"], exceptions: [], importance: "A", primarySources: [source, osakaResidentialLand], sourceFactIds: [] },
    { ...shared, knowledgeId: "takken-k-fixed-asset-tax-new-house-reduction", claim: "一定の新築住宅は、住宅部分120平方メートルまでの固定資産税額の2分の1が、一般住宅は3年間、3階建て以上の耐火・準耐火住宅は5年間減額される。", conditions: ["法定要件を満たす新築住宅であること"], exceptions: ["認定長期優良住宅には別の期間特例がある。"], importance: "B", primarySources: [source, osakaHouse], sourceFactIds: [] }
  ];
  const allowedFactIds = new Set(["u80-f1", "u80-f2"]); const ids = new Set();
  for (const item of knowledgeItems) {
    if (ids.has(item.knowledgeId)) throw new Error(`Duplicate fixed-asset-tax knowledge id: ${item.knowledgeId}`);
    ids.add(item.knowledgeId);
    if (item.examYear !== 2026 || item.lawAsOf !== "2026-04-01" || item.factcheckStatus !== "verified") throw new Error(`Invalid knowledge item state: ${item.knowledgeId}`);
    if (item.conceptId !== "takken-concept-fixed-asset-tax") throw new Error(`Unexpected concept id: ${item.knowledgeId}`);
