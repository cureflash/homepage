...
- [x] The comparison separates development-permit timing/target action, National Land Use Planning Act post-notification actor/deadline/route/area thresholds, farmland permission/notification regimes, and the cross-regime prior-permit vs post-notification distinction.
- [x] Existing source questions `takken-q-78-001` and `takken-q-78-002` were not mapped in this extraction checkpoint.
...
Current Phase 4.2 totals: 101/113 concepts with canonical knowledge extraction complete, 100/113 fully reconciled through existing-question mapping, 551 verified knowledge items, 124 existing verified questions mapped, 372 explicit question-coverage gaps.

Exact continuation: reconcile `takken-q-78-001` and `takken-q-78-002` against `takken-concept-legal-permit-notification-comparison` and map only knowledge directly measured by each stem and four choices. Do not begin Phase 4.3.
