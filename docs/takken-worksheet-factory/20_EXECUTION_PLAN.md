...
Current checkpoint:
- [x] `takken-concept-ordinary-land-lease-term` knowledge model from source unit `land-lease-right`.
- [x] Borrowed Land and Building Lease Act Article 3 split into two atomic items: default 30-year initial term, and contractual term longer than 30 years controlling.
- [x] Verified `takken-q-52-001` mapped only to those two Article 3 items in the central question-knowledge map.

Current Phase 4.2 totals: 67/113 completed concepts, 349 verified knowledge items staged, 74 existing verified questions mapped, 252 explicit question-coverage gaps.

Exact continuation: continue with `takken-concept-land-lease-opposability` from source unit `land-lease-right`. Decompose the Borrowed Land and Building Lease Act Article 10 opposability rule into atomic verified knowledge items and map `takken-q-52-002` only where it directly tests them. Do not start Phase 4.3 yet.
