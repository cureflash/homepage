...
Latest checkpoint:
- [x] `takken-concept-lease-general`: 5 verified atomic items from Civil Code Articles 601 and 604 / canonical unit 50. The model separates the lessor's undertaking to allow use and profit, the lessee's rent-payment obligation, the lessee's return obligation at termination, the 50-year original duration cap, and the 50-year cap measured from renewal. Existing verified `takken-q-50-001` directly tests all three Article 601 contract-structure items. Existing verified `takken-q-50-002` directly tests the original-term 50-year cap. The renewal-period cap remains an explicit Phase 4.3 coverage deficit.

Current Phase 4.2 totals: 64/113 modeled concepts, 339 verified knowledge items, 71 existing verified questions mapped, 248 explicit question-coverage gaps.

Exact continuation: `takken-concept-lease-deposit-opposability`, source unit `lease-deposit-opposability`. Preserve all 248 explicit deficits and map existing verified questions only to knowledge they genuinely test. Do not start Phase 4.3 yet.
