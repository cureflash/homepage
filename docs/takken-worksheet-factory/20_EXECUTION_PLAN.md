...
Latest checkpoint:
- [x] `takken-concept-installment-cancellation`: 6 verified items. The concept separates Article 42 applicability, installment-payment default, the 30-day-or-longer reasonable period, written demand, the conditions after demand for cancellation or acceleration of unmatured installments, and invalidity of contrary special terms. Existing verified `takken-q-31-001` omits the buyer-is-not-a-broker condition necessary to establish Article 42 applicability under Article 78(2), so no new mapping was added; all six items remain explicit Phase 4.3 deficits.

Current Phase 4.2 totals: 38/113 modeled concepts, 203 verified knowledge items, 35 existing verified questions mapped, 156 explicit question-coverage gaps.

Exact continuation: `takken-concept-installment-ownership-retention`, source unit `installment-sales-ownership`. Preserve all one hundred fifty-six explicit deficits and map existing verified questions only to knowledge they genuinely test. Do not start Phase 4.3 yet.
