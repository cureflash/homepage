...
Current checkpoint:
- [x] Existing verified Unit 85 questions are reconciled against `takken-concept-real-estate-appraisal-methods` using direct-measurement mapping only.
- [x] `takken-q-85-001` maps only to `takken-k-real-estate-appraisal-methods-three-approaches`.
- [x] `takken-q-85-002` maps only to `takken-k-real-estate-appraisal-methods-three-basic-methods`.
- [x] The six knowledge items defining the process/result pairs for 原価法, 取引事例比較法, and 収益還元法 remain explicit Phase 4.3 deficits; the existing Unit 85 questions do not directly test them.
...
Current Phase 4.2 totals: 108/113 concepts with canonical knowledge extraction complete, 108/113 fully reconciled through existing-question mapping, 610 verified knowledge items, 140 existing verified questions mapped, 406 explicit question-coverage gaps.

Exact continuation: extract canonical knowledge items for `takken-concept-flat35-securitization` from primary sources, then reconcile its existing verified questions using direct-measurement mapping only. Do not begin Phase 4.3.
