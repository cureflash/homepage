...
Current checkpoint:
- [x] `takken-concept-farmland-article3` has 7 canonical atomic knowledge items sourced to the 2026-04-01 Farmland Act framework and official MAFF guidance.
- [x] Verified `takken-q-74-001` maps only to `takken-k-farmland-article3-agricultural-committee-permit`, which is the knowledge directly measured by its four choices.
- [x] Verified `takken-q-74-002` maps only to `takken-k-farmland-article3-unpermitted-act-invalid`, which is the legal-effect knowledge directly measured by its four choices.
- [x] The other five farmland Article 3 knowledge items remain explicit Phase 4.3 deficits; no rights-scope, efficient-use, regular-engagement, surrounding-use, or former-minimum-area item was over-mapped.
...
Current Phase 4.2 totals: 95/113 fully reconciled concepts, 95 concepts with canonical knowledge items, 510 verified knowledge items, 118 existing verified questions mapped, 341 explicit question-coverage gaps.

Exact continuation: extract canonical atomic knowledge items for `takken-concept-farmland-article4` from the 2026-04-01 legal framework and primary official sources. Keep Article 4 separate from `takken-concept-farmland-article5`. Do not map `takken-q-75-001` until the following evidence-reconciliation checkpoint. Do not begin Phase 4.3.
