...
Current checkpoint:
- [x] `takken-concept-fire-prevention-zones` has 6 canonical atomic knowledge items sourced to the 2026-04-01 Urban Planning Act / Building Standards Act framework and official MLIT fire-prevention guidance.
- [x] Verified `takken-q-72-001` maps only to `takken-k-fire-prevention-zones-regional-district-status`.
- [x] Verified `takken-q-72-002` maps only to `takken-k-fire-prevention-zones-requirement-varies-by-zone-scale`, which is the rule directly measured by its choices.
- [x] Purpose, Article 61 general performance wording, equivalent extension-prevention performance, and opening-detail items remain explicit Phase 4.3 deficits rather than being over-mapped.
...
Current Phase 4.2 totals: 93/113 fully reconciled concepts, 93 concepts with canonical knowledge items, 493 verified knowledge items, 114 existing verified questions mapped, 329 explicit question-coverage gaps.

Exact continuation: extract canonical atomic knowledge items for `takken-concept-national-land-use-notification` from 2026-04-01 primary sources, then reconcile existing verified questions only where they directly test those items. Do not begin Phase 4.3.
