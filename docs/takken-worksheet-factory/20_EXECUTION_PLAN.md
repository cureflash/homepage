...
Current checkpoint:
- [x] `takken-concept-real-estate-advertising-rules` has 9 canonical knowledge items grounded in the official 不動産の表示に関する公正競争規約・同施行規則.
- [x] The knowledge set separates misleading-display prohibition, exact transaction-mode terms, nearest-station/walking-time display, the 80m-per-minute rule, fractional-minute round-up, nearest/farthest display for multiple lots, metric area display, horizontal-projection land area, and building total-floor-area disclosure.
- [x] Existing verified Unit 87 questions remain unmapped in this extraction checkpoint; all 9 new items remain explicit Phase 4.3 deficits until direct-measurement mapping is completed.
...
Current Phase 4.2 totals: 110/113 concepts with canonical knowledge extraction complete, 109/113 fully reconciled through existing-question mapping, 629 verified knowledge items, 142 existing verified questions mapped, 423 explicit question-coverage gaps.

Exact continuation: reconcile the existing verified Unit 87 questions against `takken-concept-real-estate-advertising-rules`, mapping only directly tested knowledge items. Do not begin Phase 4.3.
