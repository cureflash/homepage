...
Latest checkpoint:
- [x] `takken-concept-co-ownership`: 7 verified items. Civil Code Articles 249 through 252 are modeled as whole-property use according to share, compensation for use exceeding one's share, duty of care in use, equal-share presumption, substantial changes requiring the other co-owners' consent, management decisions by majority in value, and preservation acts available to each co-owner. Existing verified `takken-q-43-001` directly tests the equal-share presumption and `takken-q-43-002` directly tests whole-property use according to share. The other five rules remain explicit Phase 4.3 coverage gaps.

Current Phase 4.2 totals: 56/113 modeled concepts, 303 verified knowledge items, 57 existing verified questions mapped, 229 explicit question-coverage gaps.

Exact continuation: `takken-concept-mortgage`, source unit `mortgage`. Preserve all two hundred twenty-nine explicit deficits and map existing verified questions only to knowledge they genuinely test. Do not start Phase 4.3 yet.
