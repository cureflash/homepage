...
- [x] Land-use-neutral scope, the two regulation-zone types, zone designation authority/procedure, permit rules for each zone, and covered works including soil/stone deposit are separated into atomic knowledge.
- [ ] `takken-q-77-001` and `takken-q-77-002` have not yet been mapped; evidence reconciliation is the next checkpoint.
...
Current Phase 4.2 totals: 100/113 concepts with canonical knowledge extraction complete, 99/113 fully reconciled through existing-question mapping, 543 verified knowledge items, 122 existing verified questions mapped, 368 explicit question-coverage gaps.

Exact continuation: reconcile verified `takken-q-77-001` and `takken-q-77-002` against `takken-concept-embankment-regulation` and map only knowledge directly measured by their stems and four choices. Do not start the next concept in the same checkpoint. Do not begin Phase 4.3.
