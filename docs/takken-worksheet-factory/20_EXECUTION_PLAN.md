...
Phase 4.1 is complete. Current task is Phase 4.2 Canonical knowledge items. Do not skip ahead to 50-question expansion, print generation, or app integration before the canonical knowledge model and existing verified-question mapping are established concept by concept.
...
- [x] `takken-concept-license-validity-renewal`: **2 verified knowledge items** established and existing verified questions `takken-q-05-001` / `takken-q-05-002` mapped non-destructively. Atomic items cover the statutory **5-year license validity** and the official renewal-application window **from 90 days before expiry through 30 days before expiry** when the business will continue. Guards now cover 5 concepts / 12 knowledge items / 7 existing verified mappings.

**Exact continuation:** continue with `takken-concept-license-transfer` and source unit `takken-gyoho-license-change-closure`; establish atomic verified knowledge items for the cases requiring免許換え and its legal effect using 2026-04-01 primary/official sources, then map existing verified Phase 2/3 evidence before proceeding to `takken-concept-license-change-closure`.
