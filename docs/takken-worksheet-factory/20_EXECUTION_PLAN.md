...
- [x] `takken-q-87-001` and `takken-q-87-002` both directly test only `takken-k-real-estate-advertising-rules-misleading-display-prohibited`; both are mapped only to that knowledge item.
- [x] The other 8 advertising-rules knowledge items remain explicit Phase 4.3 deficits because the existing Unit 87 questions do not directly test them.
...
Current Phase 4.2 totals: 110/113 concepts with canonical knowledge extraction complete, 110/113 fully reconciled through existing-question mapping, 629 verified knowledge items, 144 existing verified questions mapped, 422 explicit question-coverage gaps.

Exact continuation: extract source-verified atomic knowledge items for `takken-concept-landform-disaster-risk`, then reconcile only directly tested existing Unit 88 questions. Do not begin Phase 4.3.
