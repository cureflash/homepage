...
Latest checkpoint:
- [x] `takken-concept-apparent-agency`: 6 verified items. Civil Code Articles 109, 110 and 112 are modeled as a broad statutory overview plus atomic rules for authority-grant representation within scope, authority-grant representation beyond scope, acts beyond actual authority, acts within former authority after extinction, and acts beyond former authority after extinction. Existing verified `takken-q-40-002` is mapped only to the broad overview because it does not independently test the narrower Article 109/110/112 requirements.

Current Phase 4.2 totals: 52/113 modeled concepts, 289 verified knowledge items, 51 existing verified questions mapped, 223 explicit question-coverage gaps.

Exact continuation: `takken-concept-acquisitive-prescription`, source unit `civil-prescription`. Preserve all two hundred twenty-three explicit deficits and map existing verified questions only to knowledge they genuinely test. Do not start Phase 4.3 yet.
