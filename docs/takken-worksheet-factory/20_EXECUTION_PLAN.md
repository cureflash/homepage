...
Unit 51 correction in this run:
- [x] Removed the invalid combined `takken-concept-lease-deposit-opposability` model because it does not exist in the canonical 113-concept inventory.
- [x] Split its eight staged items into canonical `takken-concept-security-deposit` (6 items) and `takken-concept-building-lease-opposability` (2 items).
- [x] Mapped `takken-q-51-001` only to the two security-deposit items it directly tests.
- [x] Mapped `takken-q-51-002` only to the two building-lease-opposability items it directly tests.

Current Phase 4.2 totals: 66/113 completed concepts, 347 verified knowledge items, 73 existing verified questions mapped, 252 explicit question-coverage gaps.

Exact continuation: start `takken-concept-ordinary-land-lease-term` from source unit `land-lease-right`. Do not start Phase 4.3 yet.
