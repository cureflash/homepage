...
- [x] Two atomic knowledge items are verified:用途地域 is a regional district under Urban Planning Act Article 8; building-use restrictions by use district are governed by Building Standards Act Article 48.
- [ ] Existing verified `takken-q-63-001` / `takken-q-63-002` have not yet been reconciled to these knowledge items.

Current Phase 4.2 totals: 81/113 fully reconciled concepts, 82 concepts with canonical knowledge items, 416 verified knowledge items, 94 existing verified questions mapped, 282 explicit question-coverage gaps.

Exact continuation: map `takken-q-63-001` and `takken-q-63-002` only to the `use-districts` knowledge items they directly measure, then mark `takken-concept-use-districts` fully reconciled. Do not begin Phase 4.3.
