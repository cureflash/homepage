...
- [x] Existing verified `takken-q-64-001` / `takken-q-64-002` are mapped only to the six knowledge items directly measured by their verified statements.
- [x] Existing Phase 2/3 question content and validation status remain unchanged; Phase 4.3 remains unstarted.

Current Phase 4.2 totals: 83/113 fully reconciled concepts, 83 concepts with canonical knowledge items, 422 verified knowledge items, 98 existing verified questions mapped, 280 explicit question-coverage gaps.

Exact continuation: continue in canonical concept order with `takken-concept-city-planning-project` from source unit `city-planning-decisions`. Reconcile its concept boundary against the already-canonicalized city-planning-decision items before adding any new knowledge item, and do not duplicate an existing claim. Do not begin Phase 4.3.
