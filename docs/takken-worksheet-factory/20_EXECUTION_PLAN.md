...
Current checkpoint:
- [x] Reconciled `takken-concept-city-planning-project` against the already-canonicalized source unit `city-planning-decisions` before adding knowledge.
- [x] Article 15 decision-authority items remain under `takken-concept-city-planning-decision`; the four Article 59 executor/authorization-route items are now under `takken-concept-city-planning-project`.
- [x] Stable knowledge IDs, claims, conditions, exceptions, source refs and verified statuses were preserved; no duplicate or new knowledge item was introduced.
- [x] Existing verified `takken-q-64-002` is reclassified to `takken-concept-city-planning-project` with the same four direct `knowledge_refs`; `takken-q-64-001` remains under the decision concept.
- [x] Existing Phase 2/3 question content and validation status remain unchanged; Phase 4.3 remains unstarted.

Current Phase 4.2 totals: 84/113 fully reconciled concepts, 84 concepts with canonical knowledge items, 422 verified knowledge items, 98 existing verified questions mapped, 280 explicit question-coverage gaps.

Exact continuation: continue in canonical concept order with `takken-concept-development-permission` from source unit `development-permission-required`. Extract atomic knowledge items against the 2026-04-01 primary-law baseline and map only existing verified questions that directly measure them. Do not begin Phase 4.3.
