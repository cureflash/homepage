...
Current checkpoint:
- [x] `takken-concept-building-coverage-ratio` now has 9 verified atomic knowledge items against the 2026-04-01 Building Standards Act Article 53 baseline.
- [x] The items cover the definition/designated limit, fire/semi-fire 10% relaxation, corner-lot 10% relaxation, combined 20% relaxation, statutory permit relaxations, and Article 53(6) application exclusions.
- [ ] `takken-q-69-001` / `takken-q-69-002` have not yet been mapped; mapping is the next checkpoint.
...
Current Phase 4.2 totals: 88/113 fully reconciled concepts, 89 concepts with canonical knowledge items, 462 verified knowledge items, 106 existing verified questions mapped, 307 explicit question-coverage gaps.

Exact continuation: map `takken-q-69-001` and `takken-q-69-002` only to the `takken-concept-building-coverage-ratio` knowledge items that each question directly measures. Do not over-map broad wording, and do not begin Phase 4.3.
