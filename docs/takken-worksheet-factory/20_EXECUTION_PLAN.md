...
Current checkpoint:
- [x] `takken-concept-shadow-restrictions` has 9 canonical atomic knowledge items sourced to the 2026-04-01 Building Standards Act Article 56-2 framework and official MLIT shadow-restriction guidance.
- [x] Verified `takken-q-71-002` is mapped only to `takken-k-shadow-restrictions-ordinance-designated-scope`, the single knowledge item directly measured by its choices.
...
Current Phase 4.2 totals: 92/113 fully reconciled concepts, 92 concepts with canonical knowledge items, 487 verified knowledge items, 112 existing verified questions mapped, 325 explicit question-coverage gaps.

Exact continuation: extract canonical knowledge items for `takken-concept-fire-prevention-zones` from its source unit and 2026-04-01 primary sources, then reconcile only genuinely measured existing verified questions. Do not begin Phase 4.3.
