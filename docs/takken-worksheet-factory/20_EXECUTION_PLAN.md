...
Current checkpoint:
- [x] `takken-concept-heirs-order` is complete. `takken-q-56-001` maps only to spouse-always-heir and the three blood-relative rank items it directly tests. `takken-q-56-002` maps only to the spouse/child one-half shares item it directly tests.
- [ ] Start `takken-concept-statutory-inheritance-shares` from `inheritance-heirs-shares` and extract only knowledge belonging to that concept before mapping direct verified-question evidence.

Current Phase 4.2 totals: 72/113 completed concepts, 369 verified knowledge items, 82 existing verified questions mapped, 253 explicit question-coverage gaps.

Exact continuation: start canonical knowledge extraction for `takken-concept-statutory-inheritance-shares` from source unit `inheritance-heirs-shares`; reuse existing verified questions only where they directly test each item. Keep Phase 4.3 blocked.
