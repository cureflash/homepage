...
Current status: planning structure created. Existing first game is the prefecture-name -> Japan-map click quiz under `social-quiz/`.

Next start point:

1. Read latest `main`.
2. Read all files under `docs/social-quiz-factory/`.
3. Inspect current `social-quiz/js/core/quiz-engine.js`, renderers, game data, and page wiring.
