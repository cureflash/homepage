...
- [x] Author p.024 `pc1.harmony.judd_p024` pending-first and independently QA all 4 finite records: 4/4 verified, pending 0, A/B/C/D=1/1/1/1; wording correction rechecked; current-question CI `33756136445`; PR #632 merged as `57163579879253d6db9529f268d73ec15f8c3467`.
- [x] Audit p.025 `シュヴルールの色彩調和論` as `SOURCE_RESOLVED` for the narrow four-relation state space directly exposed by the current AFT official Grade 1 first-stage sample.
- [x] Author p.025 `pc1.harmony.chevreul_p025` pending-first and independently QA all 4 finite records: 4/4 verified, pending 0, A/B/C/D=1/1/1/1. Do not waterfill beyond the official sample-exposed state space.

## Exact next start point
Pass the current-head p.025 dedicated/Power Color CI and merge the p.025 checkpoint if green/current. Then continue in official order to p.027 `ルードの色彩調和論` and source-audit it before any authoring. Preserve all Grade 2/3 incomplete states and source/display gates.
