...
Do not repeat the exhausted generic J-STAGE/CiNii/university-repository/KAKEN searches. The candidate conventional contrast-tone grouping is now independently corroborated at level 3, so the remaining blocker is authority elevation rather than rule discovery. Obtain a level-1/2 source that explicitly confirms the complete grouping/matrix: current official Grade 3 text pp.84–89 or equivalent JCRI/Japan Color Enterprise material, current licensed PCCS Color Calc/related first-party output, or a genuinely new peer-reviewed/university source enumerating all 12 chromatic tone memberships. Use the 2001 Japan Color Science Association table only to cross-check individual pairs. Generate `pc3.relation.tone_difference` only after the candidate matrix is confirmed by level-1/2 evidence under `42_RELIABLE_SECONDARY_SOURCE_AMENDMENT.md`.
