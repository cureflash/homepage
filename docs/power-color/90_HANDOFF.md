...
Do not repeat the exhausted generic J-STAGE/CiNii/university-repository/KAKEN searches. The next useful route is a genuinely new authoritative lead that can enumerate the conventional Grade 3 PCCS contrast-tone rule completely: current official-text-derived material from Color Certification Association/JCRI/Japan Color Enterprise, or peer-reviewed/university research that explicitly states the full tone groups or pair-membership rule. Generate `pc3.relation.tone_difference` only if that complete rule is established and independently cross-checked under `42_RELIABLE_SECONDARY_SOURCE_AMENDMENT.md`; do not infer missing pairs from tone-map appearance, generic study tables, RGB/HEX, screenshots, or isolated examples such as `b` / `dk`, `p` / `dp`, `g` / `v`, `vivid` / `White`, or `deep` / `pale`.
