...
Do not repeat the exhausted generic J-STAGE/CiNii/university-repository/KAKEN searches. The next useful route is to obtain an explicit readable rule/table behind the first-party Japan Color Research `色の基本` / PCCS instructional materials, or a genuinely new peer-reviewed/university source that enumerates all 12 chromatic tone memberships. The newly inspected 2001 Japan Color Science Association table may be used only to cross-check individual pairs; do not infer the missing matrix from its experimental subset. Generate `pc3.relation.tone_difference` only if the complete conventional Grade 3 rule is established and independently cross-checked under `42_RELIABLE_SECONDARY_SOURCE_AMENDMENT.md`.
