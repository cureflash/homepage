...
- 2026-08-26: Advanced to formal high-school course `物理基礎` after re-verifying the current MEXT `運動の表し方` and `直線運動の加速度` scope. Published the first 60 focused numerical worksheets: displacement/average-velocity 20, acceleration/velocity-change 20, and initial/final-velocity 20, with 10 direct/basic and 10 reverse variants per relation family. Every entry uses `formal_course=物理基礎` and `grade=null`; no fixed high-school year was invented. The batch reused the shared `product`/`sum` formula helper, independent recomputation, normalized-hash duplicate checks and common PDF/catalog path. After correcting only the standalone test import path, Actions run 32906657781 passed and generated publication commit `0473d105e3f0e9859d21f3ae251f5efd8aa307ca`. Published physics is now 348 worksheets total. Phase 3 remains open for the rest of `物理基礎` and advanced `物理`.
