...
## 今回進捗
Topic 08の制作前EXAM_ALIGNMENTを完了した。
...
実車での小型軽量化理由はJR東海・メーカー一次資料に基づいて説明する。N700S主電動機の極数、回転速度、質量、寸法、削減率等は一次資料確認前に推測しない。単純に「6極にすれば小さくなる」と一般化しない。
...
## 次に行う
Topic 08の解説本文＋基礎・本試験標準・複合の3段階例題を作成する。

固定4問・5小問から逆算した要求事項を本文へ全件マッピングする。N700Sへの接続はJR東海・メーカー一次資料を確認してから記述し、未確認実車値を入れない。
