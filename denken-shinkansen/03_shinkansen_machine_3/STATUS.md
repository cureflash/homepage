...
- next_start: Topic 02 の選定6問を保存済み正答を先に見ず、教材内容だけで独立再解答し、公式解答と照合して品質ゲートを判定する
...
- [ ] 02 初代0系はどうやって走っていた？ — 制作前EXAM_ALIGNMENT、解説本文、3段階例題、解説PDF、12問の練習問題・完全解説、練習PDF、解説画像PowerPointまで完了。R8上問1・2、R7下問1・2、R7上問1・2の要求事項を練習問題・PowerPointへマッピング済み。完成後独立再解答は未実施
- [ ] 03〜16 — 未着手
...
- 図表は確定済みsourceだけに基づく自作模式図で構成し、外部画像は引用していない
- 0系主電動機の未確認形式・定格値は追加せず、Topic 03の体系的な速度制御も先取りしていない
- 全20枚をレンダリングし、文字欠け・重なり・クリッピング・可読性を目視確認してPASS。overflow検査もPASS
...
## 02 QA判定
制作前 `EXAM_ALIGNMENT`、解説本文・3段階例題、解説PDF、12問の練習問題・完全解説、練習PDF、解説画像PowerPointまで完了。PowerPointは全20枚のレンダリングQAとoverflow検査までPASS。テーマ自体は未完了で、完成後独立再解答が残る。完成数は1/16のまま。

## 次
Topic 02 `初代0系はどうやって走っていた？` の選定6問（R8上問1・2、R7下問1・2、R7上問1・2）を、保存済み正答を先に見ずに教材内容だけで独立再解答する。その後、公式解答と照合し、必要公式の選択・途中式・言い換え対応・正答理由・教材カバレッジを確認して品質ゲートを判定する。