...
- next_start: Topic 07の解説PDFを作成し、全ページ表示QAと文字抽出QAを行う。完成済み解説本文・3段階例題・固定EXAM_ALIGNMENT 5問のマッピングをそのまま反映し、固定範囲外論点・未確認実車値を追加しない
...
- [ ] 07 GTO→IGBT→SiCで何が変わった？ — 制作前EXAM_ALIGNMENT・解説本文＋3段階例題完了
- [ ] 08〜16 — 未完了
...
半導体物性の詳細には深入りしない。バンド構造、結晶構造、キャリア移動度等を独立論点として追加しない。新幹線側は系列SPECの世代イメージを入口とし、実際の採用素子・回路方式は一次・技術資料で確認した内容だけを記述する。未確認実車値は追加しない。
...
- 3段階例題完成
- 固定5問の本文マッピング `5 / 5`
- 新幹線側GTO→IGBT→SiCの採用事実を一次・技術資料で確認
- 固定範囲外追加 `0件`
- 未確認実車値追加 `0件`
