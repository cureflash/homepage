...
- current_status: `topic_03_independent_reanswer_failed_scope_mismatch`
- last_completed_topic: `02 初代0系はどうやって走っていた？`
- active_topic: `03 直流モーターはどうやって速度を変える？`
- next_start: R4上問1・R3問7をTopic 03の確定範囲だけで全設問を解ける公式過去問へ差し替え、EXAM_ALIGNMENTと成果物内の対応表を修正する
...
- [ ] 03 直流モーターはどうやって速度を変える？ — 必須成果物は完成。完成後独立再解答を実施し選択肢は公式と5/5一致したが、完成教材だけで完結したのは3/5。R4上問1・R3問7が確定範囲外を含むため品質ゲートFAIL、未完了
- [ ] 04〜16 — 未着手
...
2. R4上 機械 問1 — 分巻電動機の始動抵抗、界磁・抵抗・電圧制御、静止レオナード。独立QAで逆転・回生制動の範囲外部分が判明し、差し替え必要
3. R3 機械 問7 — 分巻電動機の始動電流抑制、電機子回路の直列始動抵抗と段階的切外し。独立QAで誘導機・同期機の始動法を含む範囲外部分が判明し、差し替え必要
4. R2 機械 問1 — 他励電動機の電機子・界磁独立制御、`T=kΦIa`、弱め界磁と速度
5. R1 機械 問1 — 永久磁石直流電動機の電圧変更、一定トルク・一定磁束条件での速度計算

差し替え候補として、R7下 機械 問2（永久磁石直流電動機の銅損・逆起電力・速度）とH30 機械 問1（直流電動機の始動抵抗・逆起電力・段階的抵抗切外し）は、Topic 03の確定範囲だけで独立解答できることを確認済み。
...
- 選定5過去問の要求事項を本文節・例題へマッピング済み。ただし完成後QAでR4上問1・R3問7の全設問がTopic 03内では完結しないことを確認
...
- R6上問2、R4上問1、R3問7、R2問1、R1問1の要求事項をマッピング
...
- R4上 問1 → 独立4 / 公式4 / 教材のみFAIL。選択肢確定に逆転時の電流反転知識が必要で、回生制動も含む
- R3 問7 → 独立1 / 公式1 / 教材のみFAIL。誘導機・同期機の始動法を含む
...
## 03 QA判定
`FAIL / IN_PROGRESS`。必須成果物は揃っているが、完成後独立再解答で選定5問中2問がTopic 03の確定範囲外を含むことを検出した。仕様外内容を追加して通過させず、EXAM_ALIGNMENT側を修正する。完成数は2/16のまま。

## 次
R4上問1・R3問7を、Topic 03の確定範囲だけで全設問を解答できる公式過去問へ差し替える。候補はR7下 機械 問2とH30 機械 問1。EXAM_ALIGNMENTと成果物内の対応表を必要な範囲だけ修正した後、差し替え後5問で完成後独立再解答を再実施する。