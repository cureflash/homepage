...
- next_start: Topic 16の解説source本文を作成する
...
- [ ] 16 同期機って新幹線には使わないの？ — EXAM_ALIGNMENT固定 / IN_PROGRESS
...
- 次工程: 固定した6問・6答案要素を変更せず、解説source本文を作成する
...
## 今回進捗
Topic 16「同期機って新幹線には使わないの？」の制作前EXAM_ALIGNMENTを作成した。系列SPECの固定範囲だけで公式過去問6問・6答案要素を選定し、問題要求を独立に再確認した後、電気技術者試験センター公式解答と照合して `6 / 6 PASS`。始動法・制動巻線、短絡比、無負荷・短絡特性を使う試験法、同期調相機など固定範囲外の論点は品質ゲートから除外し、仕様外追加は0件。完成後blind独立再解答のため、制作前記録に正答選択肢番号・計算問題の最終数値は保存していない。

完成数は `15 / 16`。次工程はTopic 16の解説source本文。