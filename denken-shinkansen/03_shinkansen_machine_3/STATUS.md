...
## 06 解説source
`topics/06_power_electronics/06_power_electronics.md` の解説本文＋3段階例題を完成。

- パワー半導体: ダイオード、サイリスタ、GTO、IGBTの制御性と回路内役割
- 電力変換分類: AC→DC、DC→AC、DC→DCと直流中間回路
- 単相整流: R-L負荷、電流継続、環流ダイオード、波形判定
- 三相整流: `V_LL=√3V_s`、6パルス、最大・最小・平均値、指定脈動率
- 直流チョッパ: `T=1/f_s`、`D=t_on/T`、`Vavg=DE`
- 単相ブリッジ電圧形インバータ: IGBT/逆並列ダイオードの通流経路、`τ=L/R`
- PWM・VVVF: 電圧・周波数制御と `Ns=120f/P`
- 新幹線接続: 系列SPEC記載の主変圧器→コンバータ→直流中間回路→インバータ→誘導電動機だけを使用
