...
- R8上 理論 問10: RL一次過渡。指数関数・時定数・初期値/最終値・波形変化
...
5問から逆算した必須説明18項目を主sourceへ記録済み。

参考教材は複数系統を確認済み。
- 電気技術者試験センター公式過去問題
- e-sysnet: 電磁誘導、水力・揚水発電、三相同期電動機
...
R8上 理論 問10は Topic 10でも品質ゲートに使用済みだが、Topic 12では指数関数・時定数を複数の既習数学から選択する総合確認として再利用する。同一問題を別年度問題として重複計上するものではない。
...
公式解答・保存済み正答の個別番号・数値解は本段階で記録していない。

固定EXAM_ALIGNMENT変更0件。伝達関数等の未導入数学追加0件。法規固有論点追加0件。未確認新幹線実値追加0件。
