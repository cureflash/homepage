...
Topic 07 `微分② 速度を微分すると加速度` は制作前EXAM_ALIGNMENTまで完了。公式過去問5問を品質ゲート対象として固定し、仕様外論点・再出題重複による件数合わせはしていない。
...
公式解答は順に `(3),(5),(4),(1),(5)`。制作前独立計算は未実施であり、次工程で保存済み正答を先に見ずに5問を独立解答してから公式解答と照合する。
...
固定5問を保存済み正答を先に見ずに独立計算・判定し、公式解答と照合する。

`5 / 5 PASS` を確認するまでは解説本文制作へ進まない。仕様外論点追加、再出題の重複カウント、未確認新幹線実値の追加はしない。
