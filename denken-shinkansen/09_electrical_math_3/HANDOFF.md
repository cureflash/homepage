...
Topic 08 `積分 速度曲線の面積は距離` は制作前EXAM_ALIGNMENTまで完了。Topic 08の固定範囲だけで積分操作が解法に直接必要な公式過去問5問を固定し、積分が正答の必須経路でない候補や後続Topicの専門論点は除外した。
...
公式解答は source Markdown に記録済み。制作前独立計算はまだ実施していない。
...
固定5問を、保存済み公式解答を根拠に式を逆算せず独立に解く。その後、電気技術者試験センターの公式解答と照合し、各問がTopic 08の固定範囲だけで完結するかを判定する。

PASSなら制作前独立計算記録をsourceへ追記し、STATUS・HANDOFFを `topic_08_preproduction_independent_check_complete` へ進める。FAILがあれば、仕様外知識を足して通さず、固定過去問の妥当性またはTopic 08内の必須説明不足を切り分ける。