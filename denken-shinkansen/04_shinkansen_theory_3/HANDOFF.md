...
Topic 05 `パンタグラフから入った電気はどこへ行く？` は解説本文＋3段階例題まで完了。
...
確定済みEXAM_ALIGNMENTから逆算して、Topic 05の解説本文＋3段階例題を完成した。
...
本文では6問の要求事項をすべて教材内節へマッピングした。3段階例題は、直並列＋分圧分流、複数電源KCL＋重ね合わせ、テブナン＋ノートン＋最大電力を扱う。
...
`topics/05_dc_network/05_dc_network.md` の確定本文から解説PDFを作成する。選定6過去問への対応関係を落とさず、3段階例題を収録する。系列SPEC固定範囲外の論点・未確認実車値は追加しない。
