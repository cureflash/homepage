...
Topic 06 `0系の変圧器は何をしていた？` は解説本文＋3段階例題まで完了。次は解説PDF。
...
- 令和8年度上期 理論 問3: `Ψ=LI`、`W=LI²/2` → §6、§8、例題2
- 令和7年度下期 理論 問3: `L=NΦ/I` → §6、例題1
- 令和6年度上期 理論 問3: 磁気諸量の単位 → §2〜§4、§11
- 令和5年度上期 理論 問3: 起磁力・磁気抵抗・磁気回路 → §3〜§4、例題3A
- 令和4年度上期 理論 問3: 相互インダクタンス・結合係数 → §7、例題3B

本文にはファラデーの法則・レンツの法則、自己誘導・相互誘導、磁気エネルギー、理想変圧器巻数比まで系列SPEC固定範囲を収録した。後続Topic論点、変圧器損失・詳細等価回路、未確認0系実車値は追加していない。
...
完成したsourceの本文・3段階例題・選定5過去問への解法接続をPDFへ反映する。完成後独立再解答はまだ行わない。
