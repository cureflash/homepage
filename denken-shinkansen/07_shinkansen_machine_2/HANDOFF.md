...
制作前EXAM_ALIGNMENTと公式解答照合付き要求事項独立検証まで完了。解説本文、解説PDF、練習PDF、解説画像PowerPointは未作成。
...
降圧チョッパ、能動フィルタの補償設計、サイクロコンバータ固有制御、回生・逆変換、同期電動機、PWM・インバータはTopic 04へ追加しない。

200系の回路定数・制御角・主電動機電流等は一次資料未確認のため、現時点で実車値として置いていない。
...
固定EXAM_ALIGNMENTの12小問の要求事項を本文へ全件マッピングし、次を欠落させない。
