...
周波数変更比較では、同期速度だけでなく `X=2πfL` によりリアクタンスも変わるため、後続の計算プログラムでは電圧・抵抗・インダクタンスの固定条件を明記し、暗黙にリアクタンス一定とはしない。これはSPECの「周波数変更時の比較」を誤った条件で作らないための計算条件整理である。

## 次に行うこと

Topic 06の解説本文＋3段階例題を作成する。

固定した20小問から逆算し、同期速度・すべり・二次周波数、トルク式、最大トルク条件、停動トルク、比例推移、二次抵抗・電圧・周波数変更の影響を、固定範囲内だけで説明する。計算プログラム・比較グラフは後続工程で作成する。