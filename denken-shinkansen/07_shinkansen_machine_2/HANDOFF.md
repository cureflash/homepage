...
current status: `topic_11_exam_alignment_complete`
...
- 公式問題・公式解答／標準解答URLを記録
- 要求知識・式・設問型、教材マッピング予定、固定範囲境界を記録
- 参考教材2系統以上を記録
- `e-sysnet.com` は直接対応する公開ページを確認できず、件数合わせで採用していない
- 制作前独立検証: `15 / 15 PASS`
- 完成後独立再解答は未実施。教材完成後に別ゲートとして行う

## 次に行うこと
Topic 11「GTO→IGBT→SiCの損失を計算する」の解説本文＋3段階例題を作成する。

1. 固定5問・15答案要素とTopic 11固定範囲を変更しない。
2. 導通損失、スイッチング損失、`P_sw=f_sw(W_on+W_off)`、素子数を含む総損失集計を、式の意味・単位・成立条件・解法手順まで説明する。
3. H22二次型のIGBT／逆並列ダイオード導通区間・平均電流を、導通損失計算へ接続する。
4. GTO・IGBT・SiCの実機比較値はメーカー・鉄道事業者・技術論文等で確認し、実在値／計算値／仮定値を区別する。
5. 基礎・本試験標準・複合の3段階例題を作成し、固定15答案要素を本文へ全件マッピングする。