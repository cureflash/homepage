...
今回、練習PDFまで完成した。A4縦10ページ、一次試験型8問（五肢択一）＋二次試験型4問（記述式）の計12問。全問に完全解説を付けた。

二次試験型では、H24二次問1 (1)〜(4)型の解法骨格である「損失→電機子抵抗→逆起電力→速度→部分負荷端子電圧」を独自問題で反復し、使用式、前提、途中式、単位、検算まで答案として残せる形にした。

全数値問題は教材用仮定値であり、0系実車値ではない。全10ページを200 dpiでレンダリングして表示QAし、文字切れ・欠損・ページ外はみ出しなし。数値解答も別計算で確認済み。
