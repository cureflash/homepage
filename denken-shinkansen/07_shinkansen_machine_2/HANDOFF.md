...
- 内部起電力・端子電圧・同期リアクタンスは等価回路・フェーザの記号認識と基本関係までに限定する。
...
## Topic 13 練習問題source
- `topics/13_l0_linear_synchronous_motor_equivalent_circuit/13_l0_linear_synchronous_motor_equivalent_circuit_practice.md`: `completed`
- 一次試験型10問: 全問五肢択一
- 二次試験型5問: 記述・途中式または論証付き
- 一次正答一意性: `10 / 10 PASS`
- 独立計算・論理QA: `15 / 15 PASS`
- 固定5過去問・22答案要素: `22 / 22` 接続
- 同期速度、同期リアクタンス、発電機／電動機フェーザ計算: 再検算PASS
- 固定範囲外追加: `0件`
- Topic 14先取り: `0件`
- 未確認L0実値追加: `0件`
...
- `topics/13_l0_linear_synchronous_motor_equivalent_circuit/13_l0_linear_synchronous_motor_equivalent_circuit_images.pptx`: `completed`
- 16:9・4枚
- 同期機→リニア同期モータの概念対応、電機子反作用、`X_s=X_a+X_l`、1相分等価回路・フェーザ、固定5過去問への接続を可視化
- Visual QA: `4 / 4 PASS`（1601×900）
- overflow: `0件`
- ZIP整合性: `PASS`
- 固定一次4問＋二次1問・22答案要素: 維持
