...
## 今回進めた内容
Topic 01 `0系の主変圧器を等価回路で解剖する` の制作前EXAM_ALIGNMENTを作成した。

選定した公式過去問は、R7一次機械問3、R6二次機械・制御問2、R3二次機械・制御問2、R2二次機械・制御問2、H28二次機械・制御問2、H27二次機械・制御問2の6問。

要求論点は、一次・二次換算、励磁枝、巻線抵抗・漏れリアクタンス、無負荷試験・短絡試験、%インピーダンス、電圧変動率、鉄損・銅損、規約効率・最大効率、負荷率との関係に整理した。

R7一次機械問3とR6二次機械・制御問2は試験センター公式PDFで再確認した。同一論点を扱う `05_shinkansen_vehicle_2` Topic 01 の既存EXAM_ALIGNMENTとも照合し、系列間で要求論点が矛盾しないことを確認した。

0系実車値は、既存三種教材で一次資料確認済みの交流60 Hz 25 kV、主変圧器容量1650 kVA、低圧タップ切換、25段を再利用可能とした。公開一次資料で確認できていない0系固有の等価回路定数、%インピーダンス、鉄損・銅損等は実値として扱わない。
