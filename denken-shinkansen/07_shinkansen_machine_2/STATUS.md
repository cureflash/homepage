...
- next_start: Topic 06の解説PDFを作成し、全ページ表示QAを行う
...
- [x] 系列SPEC確認
- [x] MASTER_SPEC / EXAM_ALIGNMENT_SPEC確認
- [x] R08一次問2を確認し、等価回路定数測定法は固定範囲外として非採用
- [x] 公式過去問を一次2問＋二次記述3問、計5問選定
- [x] 混合問題は固定範囲へ直接対応する小問だけをゲート対象として明示
- [x] 参考教材を e-sysnet＋電験王2 の複数系統で確認
- [x] source Markdownへ制作前EXAM_ALIGNMENTを記録
...
- [x] 系列SPEC確認
- [x] MASTER_SPEC / EXAM_ALIGNMENT_SPEC確認
- [x] 公式過去問を一次2問＋二次記述4問、計6問選定
- [x] 混合問題は固定範囲へ直接対応する小問だけをゲート対象として明示
- [x] H25一次問1の二重かご形詳細、H23二次問1の逆相制動等を固定範囲外として除外
- [x] 参考教材を e-sysnet＋電験王2 の複数系統で確認
- [x] source Markdownへ制作前EXAM_ALIGNMENTを記録
- [x] ゲート対象20小問の公式解答照合を伴う要求事項の制作前独立検証: 20 / 20 PASS
- [x] 解説本文＋3段階例題
- [ ] 解説PDF
- [ ] 練習PDF（解答・完全解説付き）
- [ ] 解説画像PowerPoint
...
## 今回進めた内容
Topic 06「誘導電動機の最大トルクはどこで出る？」の解説本文＋3段階例題を完成した。

確認内容:
- 固定EXAM_ALIGNMENT: 一次2問＋二次記述4問、計6問・20小問を維持
- 制作前独立検証: `20 / 20 PASS` を維持
- 教材で欠落不可の12項目を本文へマッピング
- 同期速度・すべり・二次周波数、トルク式の導出、最大トルク条件、停動トルク、比例推移を固定範囲内で説明
- 二次抵抗変更では `s_m∝r_2'` と最大トルク値不変を区別
- 電圧変更では固定条件を明示して `T∝V_1^2` を説明
- 周波数変更では `n_s∝f` に加え、インダクタンス一定なら `X=2πfL` も変化することを明示
- 基礎・本試験標準・複合ひっかけの3段階例題を作成し、途中式・単位・検算観点を収録
...
## 判定
Topic 06は `topic_06_explanation_source_complete`。完成数は `5 / 22` のまま。次は解説PDF＋全ページ表示QA。