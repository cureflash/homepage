...
- next_start: Topic 38の練習sourceを正本として練習PDFを制作し、全ページ表示・文字抽出・問題/正答/解説整合・固定5問5答案要素の反映をQAする

## 新品質ゲート進捗
- [x] 01〜37 — PASS / COMPLETED
- [ ] 38 — PRACTICE_SOURCE_COMPLETE / PASS。練習PDF以降は未制作
...
## Topic 38 今回完了した1段階
練習sourceを制作・QAし、`PASS / PRACTICE_SOURCE_COMPLETE`。
...
JR東日本公開資料で確認できた「システム信頼度の向上」「事故時の情報収集・制御・早期復旧」だけを実システム事実として扱う。COSMOS/COSMOS-SCADAのサーバ台数、二重系/三重系、待機方式、通信経路、切替時間、切替条件、同期方式は推測しない。練習sourceの信頼度数値は学習用仮定値であり、COSMOS/COSMOS-SCADAの実信頼度とは扱わない。

## 完成数
`37 / 38`。Topic 38は練習sourceまで完了。次は練習PDF制作・QA。
