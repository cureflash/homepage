...
- next_start: Topic 25の解説本文＋3段階例題を作成する。確定済み5問・7小問の要求事項を全件カバーし、系列SPEC固定範囲と一次資料確認済み事実だけを使用する

## 新品質ゲート進捗
- [x] 01〜24 — PASS
- [ ] 25 L0系④ 誘導集電 — EXAM_ALIGNMENT完了、制作中
...
今回確認した一次資料から実機コイル寸法、実運転周波数、定格電力、電圧・電流、伝送効率の具体値は確定しないため、実車値として追加しない。
...
## 次
Topic 25 `L0系④ 誘導集電` の解説本文＋3段階例題を作成する。確定済み5問・7小問の要求事項を全件カバーし、系列SPEC固定範囲と一次資料確認済み事実だけを使う。