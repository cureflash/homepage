...
- next_start: Topic 38の解説画像PowerPointを制作し、固定5問5答案要素の可視化、16:9表示、overflow、PPTX ZIP整合性をQAする

## 新品質ゲート進捗
- [x] 01〜37 — PASS / COMPLETED
- [ ] 38 — PRACTICE_PDF_COMPLETE / PASS。PowerPoint以降は未制作
...
## Topic 38 今回完了した1段階
練習PDFを制作・QAし、`PASS / PRACTICE_PDF_COMPLETE`。
...
### Topic 38 source/PDF/練習source・練習PDFで反映した試験論点
...
JR東日本公開資料で確認できた「システム信頼度の向上」「事故時の情報収集・制御・早期復旧」だけを実システム事実として扱う。COSMOS/COSMOS-SCADAのサーバ台数、二重系/三重系、待機方式、通信経路、切替時間、切替条件、同期方式は推測しない。練習source/PDFの信頼度数値は学習用仮定値であり、COSMOS/COSMOS-SCADAの実信頼度とは扱わない。

## 完成数
`37 / 38`。Topic 38は練習PDFまで完了。次は解説画像PowerPoint制作・QA。
