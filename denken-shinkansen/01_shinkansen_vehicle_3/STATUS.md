...
- next_start: Topic 26の制作前EXAM_ALIGNMENTを実施する。系列SPEC固定範囲から関連する公式過去問を直近年度優先で原則5問以上調査し、制作前独立解答と教材要求事項を確定する

## 新品質ゲート進捗
- [x] 01〜25 — PASS
- [ ] 26 補助電源① 主回路の電気を車内設備へ — 未着手

## 25 EXAM_ALIGNMENT
品質ゲート対象は5問・7小問。制作前解答検証7 / 7 PASS、完成後独立再解答も7 / 7 PASS。
...
## 25 完成後独立再解答
完成教材に収録した式・適用条件・解法手順だけを使い、保存済み正答を先に見ずに選定5問・7小問を再解答し、公式解答と照合した。
...
結果: 7 / 7 PASS。必要公式の選択、適用条件、途中計算を完成教材内だけで再構成でき、公式解答と全件一致。仕様外知識や未確認実車値による補完は不要だった。Topic 25をcompletedとし、完成数を25 / 38へ更新した。
...
## 次
Topic 26 `補助電源① 主回路の電気を車内設備へ` の制作前EXAM_ALIGNMENTを実施する。系列SPEC固定範囲から関連する公式過去問を直近年度優先で原則5問以上調査し、制作前独立解答と教材要求事項を確定する。
