...
- next_start: Topic 05の練習PDF。固定5過去問の要求事項を反映し、三種仕様どおり原則12問以上・本試験形式中心・完全解説で作成し、全ページ表示QAを行う
...
- [ ] 05 新幹線を再エネだけで走らせられる？ — 制作前EXAM_ALIGNMENT、解説本文・3段階例題、解説PDF完了。練習PDF以降は未着手
- [ ] 06〜16 — 未完了

## Topic 05 現在地
source: `topics/05_renewable_energy/05_renewable_energy.md`

制作前EXAM_ALIGNMENTで固定した公式過去問5問を変更せず、要求事項から逆算して解説本文＋3段階例題を完成済み。今回、解説PDF `topics/05_renewable_energy/05_renewable_energy_explanation.pdf` をA4縦5ページで作成し、本文・主要数式・3段階例題・固定5過去問への対応を収録した。200 dpiで全5ページをレンダリング確認し、文字欠け・重なり・ページ外はみ出しなしでPASS。未確認の列車消費電力・再エネ設備容量・蓄電容量は実値化しておらず、SPEC固定範囲外の論点も追加していない。
...
本文・PDFでは太陽光、風力、地熱、燃料電池、蓄電池、設備利用率、系統連系、需給バランス、新幹線への接続をSPEC固定範囲内で収録。
...
固定範囲外のバイオマス、小水力、洋上風力の直流送電等は追加していない。地熱発電はSPEC固定範囲なので本文・PDFへ収録したが、品質ゲート対象過去問は変更していない。
...
## 次
Topic 05の練習PDFを作成する。固定5過去問の要求事項を外さず、三種仕様どおり原則12問以上、本試験形式を中心に完全解説を付け、全ページ表示QAを行う。仕様外論点は追加しない。完成数は `4 / 16` のまま。