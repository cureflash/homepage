...
## 今回進捗
Topic 08 `新幹線の変電所はなぜ数十kmおき？` を1段階進め、練習PDFを完成した。

`topics/08_voltage_drop/08_voltage_drop_practice_source.md` を正本として `topics/08_voltage_drop/08_voltage_drop_practice.pdf` を作成した。A4縦4ページ、全12問・全問五肢択一、基礎3／本試験標準7／複合2、完全解説付き。200 dpi全4ページ表示QA `4 / 4 PASS`、文字抽出QA PASS、置換文字 `U+FFFD` 0件。数値・論理再検算 `12 / 12 PASS`、正答一意性 `12 / 12 PASS`、固定5問・7小問への接続 `7 / 7`。SPEC固定範囲外論点・未確認実設備値の追加は0件。
...
固定5問・7小問の要求事項を可視化し、全スライド表示QA・overflow・PPTX整合性を確認する。Topic 09以降の論点・未確認の実設備値は追加しない。
