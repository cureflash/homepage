...
## 今回進捗
Topic 03を1段階進め、制作前 `EXAM_ALIGNMENT` を完了した。公式過去問を直近年度から確認し、`SPEC.md` の固定範囲に適合する6問・7小問を品質ゲート対象として固定。制作前独立再解答は `7/7 PASS`。固定範囲外の燃焼化学、詳細煙風道設備、保護装置等は追加していない。
...
Topic 03の川崎発電所等は、次段階で一次資料を確認するまで実設備値を置かない。
...
## 次の正確な開始点
Topic 03「JR東日本はなぜ火力発電所まで持っている？」の解説本文と3段階例題を作成する。`topics/03_thermal_power/03_thermal_power.md` の `EXAM_ALIGNMENT` 6問・7小問を教材だけで解ける中間知識を欠落させず、`SPEC.md` の固定範囲から仕様を追加しない。川崎発電所等の実設備事実を使う場合はJR東日本等の一次資料で確認できたものだけを採用する。