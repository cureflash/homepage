...
- next_start: Topic 08の解説本文＋3段階例題。制作前に固定した一次3問＋二次2問・20対象項目を全件本文へマッピングし、`SPEC.md` の固定範囲と指定4グラフだけを扱う。後続Topicを件数合わせで追加しない
...
- [ ] 08 位相制御と平均直流電圧 — EXAM_ALIGNMENT固定完了 / 解説source未作成
...
次工程はTopic 08 `位相制御と平均直流電圧` の解説本文＋3段階例題。