...
現在地は `topic_09_exam_alignment_complete`。Topic 09 `直流機方式の損失・熱設計` は制作前EXAM_ALIGNMENTを完了し、固定一次4問＋二次1問・14対象項目を設定した。制作前独立検証は一次 `11 / 11 PASS`、二次 `3 / 3 PASS`、合計 `14 / 14 PASS`。仕様外論点・後続Topicの件数合わせ採用・未確認実車値追加はいずれも0件。

次工程はTopic 09の解説本文＋3段階例題。固定した5問・14対象項目から逆算し、`SPEC.md` の「銅損、鉄損、機械損、雑負荷損、効率、温度上昇、定格、冷却、負荷率、保守」と指定3可視化だけを扱う。
...
- 2400×1350 px全4スライド Visual QA: `PASS`
- overflow: `0件`
- PPTX ZIP整合性: `PASS`
- 日本語グリフ欠落: `0件`
- 固定一次3問＋二次2問・20対象項目: 維持
...
## 次の正確な開始点
Topic 09 `直流機方式の損失・熱設計` の解説本文＋3段階例題。

1. `09_dc_machine_loss_thermal_design.md` の固定5問・14対象項目を逆算条件にする。
2. `SPEC.md` 固定範囲と指定3可視化だけを扱う。
3. 一次の語句・損失分類だけでなく、H22二次問2を教材だけで途中式付き再現できる説明を入れる。
