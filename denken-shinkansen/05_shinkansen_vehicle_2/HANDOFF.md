# HANDOFF - 新幹線車両・電験二種

更新日: 2026-09-15

## 正本・現在地
最上位正本は `../MASTER_SPEC.md` と `../EXAM_ALIGNMENT_SPEC.md`。active series は `05_shinkansen_vehicle_2`。

Topic 01〜06は最終QAまで `PASS`。完成数は `6 / 39`。

現在地は `topic_07_powerpoint_resynced`。active topicは Topic 07 `直流主回路の高出力化`。

完成後独立再解答で不足したH21一次「機械」問2(2)の「負荷損を規定の基準巻線温度へ補正」を固定範囲内で解説source・練習sourceへ補強済み。解説PDF・練習PDFに続き、本runでPowerPointへも必要最小限で同期し、180 dpi全4スライド表示QA・overflow・PowerPoint ZIP整合性を再実施してPASS。固定EXAM_ALIGNMENT、4スライド、SPEC指定3グラフ、確認済み実車値は変更していない。

直近の完成後独立再解答は補強前教材に対する `14 / 15 PASS / NEEDS_REVISION` のため、Topic 07はまだ `completed` ではない。次は補強済み完成教材だけで固定15対象項目を再独立再解答する。

## Topic 06 完了記録
固定EXAM_ALIGNMENTは一次3問＋二次2問、計5問。

- R8 一次 機械 問3 — 変圧器損失・規約効率・最大効率条件
- R5 一次 機械 問1 — 回転機の効率・機械損・鉄損・銅損・漂遊負荷損・固定損
- H30 一次 機械 問5 — 無負荷損・負荷損・鉄損・規約効率
- R2 二次 機械・制御 問2 — 鉄損・銅損・負荷率・最大効率・部分負荷効率
- H22 二次 機械・制御 問2 — 無負荷試験・銅損・最大効率条件

二次記述2問を含み、二種一次・二次合計の原則5問以上を満たす。件数合わせのR7一次機械問3、IGBT等は追加していない。

完成後独立再解答:
- 一次3問・15小問 `15 / 15 PASS`
- 二次2問・7設問 `7 / 7 PASS`
- 合計 `22 / 22 PASS`
- 教材外知識補完 `0件`
- 固定範囲外追加 `0件`
- 未確認0系実値追加 `0件`

成果物・QA:
- 解説source: 完了
- 解説PDF: A4縦5ページ、180 dpi全ページ Visual QA `PASS`
- 練習source: 一次8問＋二次4問、計算・論理 `12 / 12 PASS`
- 練習PDF: A4縦3ページ、表示・文字抽出QA `PASS`
- 解説画像PowerPoint: 16:9・4スライド、表示QA・overflow・ZIP整合性 `PASS`
- 最終QA: `PASS / completed`

## Topic 07 固定EXAM_ALIGNMENT
固定範囲は `SPEC.md` の「主回路構成比較、定格電圧、定格電流、主電動機出力、損失、冷却、編成出力、高速化との関係」と指定3グラフだけ。

固定過去問は一次4問＋二次1問、計5問・15対象項目。

- H28 一次 機械 問5 (1) — 大きな始動トルクを要求する電気鉄道と直流直巻電動機
- H26 一次 機械 問5 (1)(2) — 大きな始動時トルク、直流直巻電動機
- H22 一次 機械 問1 全小問 — 回転機の効率、定格出力、実測効率、規約効率、固定損・負荷損・漂遊負荷損
- H21 一次 機械 問2 全小問 — 無負荷損・負荷損、基準巻線温度、規約効率、負荷率 `m` に対する出力 `m S_n cosφ`、負荷損 `m^2 P_c`
- H24 二次 機械・制御 問1 (1)(2) — 電機子銅損、電機子抵抗、誘導起電力の記述計算

制作前独立検証:
- 一次 `13 / 13 PASS`
- 二次 `2 / 2 PASS`
- 合計 `15 / 15 PASS`
- 固定範囲外知識を正答根拠に使用: `0件`
- 後続Topicの位相制御・誘導機・VVVF・回生・IGBT/SiCを件数合わせで採用: `0件`

H28問5の(2)以降、H26問5の(3)以降、H24二次問1の(3)〜(6)は後続Topicのチョッパ・回生・誘導機・VVVF・IGBT等を含むため採用していない。R6・R5等の近年電気鉄道問題も後続技術が主眼のものは件数合わせで固定していない。

## Topic 07 解説source
解説本文＋3段階例題を完成し、固定15対象項目を全件本文へマッピングした。固定EXAM_ALIGNMENTは変更していない。

実車値は次の一次資料・学会資料で直接確認したものだけを採用した。

- 日立評論 1986年3月号「最新技術を応用した100系新幹線電車の電気品」
  - 0系 MT200B: 185 kW / 415 V / 490 A / 2,200 rpm / 自己通風
  - 100系 MT202: 230 kW / 625 V / 405 A / 2,900 rpm / 他力通風
- 電気学会雑誌 102巻2号（1982）200系新幹線電車解説
  - 200系 MT201: 230 kW / 475 V / 530 A / 2,150 rpm / 強制風冷14 m3/min

0系185 kWから100系・200系230 kWへの1台当たり連続定格出力増加は約24.3%。高出力化を電流増加だけで説明せず、定格電圧・定格電流・損失・冷却を同時に扱った。

本文では以下を収録済み。
- `P_in=VI`、`P_loss=P_in-P_out`、`η=P_out/P_in`
- `P_Cu=I_a^2R_a`、ブラシ損 `V_bI_a`、`E=V-I_aR_a-V_b`
- 実測効率・規約効率・固定損・負荷損・漂遊負荷損
- 負荷率 `m` と `m^2P_c`
- 規約効率に用いる負荷損は規定の基準巻線温度へ補正した値を使う
- 損失→発熱→冷却の接続
- 編成出力 `P_train=NP_m` の計算法。ただし未確認の実車搭載数は使用していない
- 高速化の一般関係 `P=Fv`
- 基礎／本試験標準／複合の3段階例題
- 指定3グラフ「世代別出力比較」「出力―損失」「出力―発熱」へ使う関係

H21一次 問2(2)補強:
- §7: 負荷損を測定時温度のまま使わず、規定の基準巻線温度へ補正して規約効率へ用いることを追加
- §8: `P_c` を基準巻線温度へ補正済みの定格電流時負荷損として扱うことを追加
- §12: 解法手順へ基準巻線温度の確認を追加
- 温度補正式、温度上昇計算、熱抵抗・時定数等の詳細熱設計: 追加 `0件`

未確認のまま真値化していないもの:
- 0系・100系・200系を同一条件で比較できる主電動機搭載数・編成定格出力
- 主回路全体損失・総合効率
- 冷却容量、温度上昇、熱抵抗
- 実車けん引力―速度特性

教材用の数値例は全て仮定値と明記した。位相制御、誘導機、VVVF、回生、IGBT/SiC、詳細熱設計の追加は0件。

## Topic 07 解説PDF

`07_dc_main_circuit_high_output_explanation.pdf` はH21一次 問2(2)補強後sourceへ同期済み。

- A4縦5ページ
- 3段階例題を途中式付きで収録
- H21一次 問2(2)の基準巻線温度補正を収録
- SPEC指定3グラフを収録
  - 世代別出力比較: 確認済み実車値 `185 / 230 / 230 kW`
  - 出力―損失: 教材用仮定値
  - 出力―発熱: 教材用仮定値
- 180 dpi全5ページ表示QA: `PASS`
- 文字抽出・Unicode置換文字確認: `PASS / 0件`
- 固定一次4問＋二次1問・15対象項目を維持
- SHA-256: `a93dca994182676ecbd02b8823efb42a4f70cf710ff7d6feac24332a0cfabd75`
- 未確認実車値の真値化: `0件`
- 固定範囲外追加: `0件`

QA記録: `07_dc_main_circuit_high_output_explanation_pdf_qa.md`

## Topic 07 練習source

`07_dc_main_circuit_high_output_practice_source.md` は一次8問＋二次4問、計12問、全問完全解説付き。

- H28一次問5(1)、H26一次問5(1)(2)、H22一次問1、H21一次問2、H24二次問1(1)(2)へマッピング
- `P_in=VI`、`P_loss=P_in-P_out`、`η=P_out/P_in`、`P_Cu=I_a^2R_a`、`V_bI_a`、`E=V-I_aR_a-V_b`、`mS_n cosφ`、`m^2P_c`、`P_train=NP_m`、`P=Fv` を固定範囲内で練習
- H21一次 問2(2)対策として一次3に基準巻線温度補正の用語判断、一次4・二次2に「`P_c` は基準巻線温度へ補正済み」とする条件・解法を追加
- 問題数・正答・数値計算変更: `0件`
- 独立計算・論理・正答一意性QA: `12 / 12 PASS`
- 教材外知識補完: `0件`
- 固定範囲外論点追加: `0件`
- 未確認実車値の真値化: `0件`

未確認の実車搭載数・編成出力・主回路総損失・効率・冷却容量・けん引力―速度特性は使っていない。数値問題の条件は教材用仮定値と明記した。

## Topic 07 練習PDF

`07_dc_main_circuit_high_output_practice.pdf` はH21一次 問2(2)補強後sourceへ同期済み。

- A4縦3ページ
- 一次8問＋二次4問、全問完全解説付き
- 一次3へ基準巻線温度補正の用語判断を同期
- 一次4・二次2へ「`P_c` は基準巻線温度へ補正済み」とする条件・解法を同期
- 問題数・正答・数値条件変更: `0件`
- 180 dpi全3ページ表示QA: `PASS`
- 文字抽出・Unicode置換文字確認: `PASS / 0件`
- `基準巻線温度` の収録確認: `PASS`
- 計算・論理: `12 / 12 PASS`
- 固定一次4問＋二次1問・15対象項目を維持
- SHA-256: `85dd16819a4d209a1e5700a2447dd6cc3412e6bd6da19c0eebd817ac130819bb`
- 教材外知識補完: `0件`
- 固定範囲外論点追加: `0件`
- 未確認実車値の真値化: `0件`

QA記録: `07_dc_main_circuit_high_output_practice_qa.md`

## Topic 07 解説画像PowerPoint

`07_dc_main_circuit_high_output_images.pptx` はH21一次 問2(2)補強後sourceへ同期済み。

- 16:9・4スライド
- SPEC指定3グラフ: 3 / 3収録
- 世代別出力比較: 確認済み実車値 `0系185 / 100系230 / 200系230 kW`
- 出力―損失: 教材用仮定モデル `P_out=230m`, `P_loss=8+12m^2`
- 出力―発熱: 同じ教材用仮定モデルで `P_heat≈P_loss`
- スライド3へ「規約効率に用いる負荷損は規定の基準巻線温度へ補正した値を使う」を同期
- 温度補正式、温度上昇計算、熱抵抗・時定数等の詳細熱設計: 追加 `0件`
- 180 dpi全4スライド表示QA: `PASS`
- overflow: `0件`
- PowerPoint ZIP整合性: `PASS`
- 日本語グリフ欠落: `0件`
- 固定一次4問＋二次1問・15対象項目を維持
- 未確認実車値の真値化: `0件`
- 固定範囲外論点追加: `0件`
- SHA-256: `77bd13135ee4e314f3146068ed02d53ae8aba02883edf4e9d4d89ea9c7665bb3`
- H21一次 問2(2)補強後sourceへの同期: `completed`

QA記録: `07_dc_main_circuit_high_output_images_qa.md`

## Topic 07 完成後独立再解答

`07_dc_main_circuit_high_output_independent_reanswer.md` は補強前教材に対する既存記録を保持する。

- 固定一次4問＋二次1問・15対象項目を保存済み正答を先に見ずに再解答
- 独立解答と公式解答・標準解答の値・語句一致: `15 / 15`
- 補強前教材のみ品質ゲート: 一次 `12 / 13 PASS`、二次 `2 / 2 PASS`、合計 `14 / 15 PASS`
- H28一次 問5(1): `1 / 1 PASS`
- H26一次 問5(1)(2): `2 / 2 PASS`
- H22一次 問1: `5 / 5 PASS`
- H21一次 問2: `4 / 5 PASS`
- H24二次 問1(1)(2): `2 / 2 PASS`
- `FAIL`: H21一次 問2(2)「負荷損を基準巻線温度へ補正」。公式解答とは一致したが、当時の完成学習教材から根拠を再構築できず外部知識補完が必要だった
- source上の不足補強: `completed`
- 解説PDF・練習PDF・PowerPointへの補強同期: `completed`
- 補強済み完成教材だけでの再独立再解答: `未実施`
- 固定EXAM_ALIGNMENT変更: `0件`
- 固定範囲外論点追加: `0件`
- 未確認実車値追加: `0件`
- 現在判定: `NEEDS_REVISION`

## 次の正確な開始点
Topic 07 `直流主回路の高出力化` の固定一次4問＋二次1問・15対象項目を、補強済み完成教材だけで再独立再解答する。

1. 保存済み正答・旧独立再解答記録を先に見ず、問題条件から答案を再構築する。
2. 完成教材だけで各設問の根拠・計算・語句を再現し、教材外知識を補完しない。
3. 各対象項目を公式解答・標準解答と照合し、一次13項目＋二次2項目を個別にPASS/FAIL記録する。
4. H21一次 問2(2)は「負荷損を規定の基準巻線温度へ補正して扱う」を完成教材だけで再現できることを確認する。
5. 1件でも教材外知識補完が必要なら `PASS` にせず、exact missing knowledgeを記録して止める。
6. 固定EXAM_ALIGNMENT、採用過去問、15対象項目は変更しない。
7. 位相制御、誘導機、VVVF、回生、IGBT/SiC、詳細熱設計を追加しない。
8. 未確認の編成出力・主回路総損失・冷却容量・けん引力特性を補完しない。
