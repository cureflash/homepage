...
現在地は `topic_10_explanation_source_complete`。Topic 10 `300系① 誘導電動機等価回路` は制作前EXAM_ALIGNMENT（一次3問＋二次2問、22答案要素、制作前独立検証 `22 / 22 PASS`）と解説本文＋3段階例題まで完成。固定5問・22答案要素を本文へ `22 / 22` マッピングし、SPEC指定4グラフへつながる計算条件を確定した。固定範囲外追加・未確認300系実車値追加は `0件`。

次工程はTopic 10の解説PDF。固定一次3問＋二次2問・22答案要素、3段階例題、SPEC指定4グラフへつながる説明を維持し、固定範囲外を追加しない。
...
## 次の正確な開始点
Topic 10 `300系① 誘導電動機等価回路` の解説PDF。

1. 完成済み解説sourceを正本としてA4縦の解説PDFを制作する。
2. 固定一次3問＋二次2問・22答案要素、3段階例題、SPEC指定4グラフへつながる説明を落とさない。
