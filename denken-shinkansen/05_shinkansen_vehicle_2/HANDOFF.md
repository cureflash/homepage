...
現在地は `topic_13_explanation_source_complete`。Topic 13 `300系④ 回生と四象限運転` は解説本文＋3段階例題まで完成。固定一次4問＋二次1問・15答案要素を本文へ `15 / 15` 接続し、SPEC指定8項目・3可視化へ接続。Topic 14先取り、回生失効・電力貯蔵設備、H24二次の無効電流・出力可能領域、SPEC外制御方式、未確認300系実車値の追加は0件。次は解説PDF。
...
## 次の正確な開始点
Topic 13 `300系④ 回生と四象限運転` の解説PDFを作成する。固定5過去問・15答案要素、3段階例題、SPEC指定3可視化を反映し、固定範囲外へ広げない。