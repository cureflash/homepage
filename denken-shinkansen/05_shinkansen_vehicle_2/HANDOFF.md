...
現在地は `topic_10_explanation_pdf_complete`。Topic 10 `300系① 誘導電動機等価回路` は制作前EXAM_ALIGNMENT（一次3問＋二次2問、22答案要素、制作前独立検証 `22 / 22 PASS`）、解説本文＋3段階例題、解説PDFまで完成。PDFはA4縦5ページ、180 dpi全5ページVisual QA・文字抽出QAともPASS。固定5問・22答案要素、3段階例題 `3 / 3`、SPEC指定4グラフ `4 / 4`を維持し、固定範囲外追加・未確認300系実車値追加は `0件`。

次工程はTopic 10の練習問題・完全解説source。固定一次3問＋二次2問・22答案要素へ接続し、一次択一と二次記述を固定範囲内で構成する。
...
## 次の正確な開始点
Topic 10 `300系① 誘導電動機等価回路` の練習問題・完全解説source。

1. 固定一次3問＋二次2問・22答案要素へ直接つながる問題を固定範囲内で設計する。
2. 一次は択一、二次は記述とし、途中式・成立条件・単位・検算を完全解説へ含める。
3. 300系実車値は未確認のため真値化せず、教材用仮定値を明示する。Topic 11以降を追加しない。