...
現在地は `topic_03_images_pptx_complete`。Topic 03 `0系③ 直流主電動機の電圧方程式とトルク` は制作前EXAM_ALIGNMENT、解説本文、3段階例題、4指定グラフ、解説PDF、練習問題・完全解説source、練習PDF、解説画像PowerPointまで完了した。次は完成教材だけによる公式過去問独立再解答。
...
## 次の正確な開始点
Topic 03 `0系③ 直流主電動機の電圧方程式とトルク` の完成教材だけを使い、H28一次 機械 問1の固定範囲(1)(2)とH24二次 機械・制御 問1の固定範囲(1)(2)(4)を、保存済み正答を見ずに独立再解答する。

教材外知識の補完が0件か、公式解答と一致するか、範囲外論点へ依存していないかを記録する。PASS後に最終QAへ進む。