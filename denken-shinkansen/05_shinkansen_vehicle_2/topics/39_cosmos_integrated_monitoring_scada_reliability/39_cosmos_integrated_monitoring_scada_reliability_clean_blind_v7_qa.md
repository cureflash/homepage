# Topic 39 clean blind v7 QA

更新日: 2026-09-21

判定: `FAIL / solver_error / CLEAN_BLIND_V8_PENDING`

## 固定条件

- 固定対象: 5問・25答案要素（一次20、二次5）
- v7 candidate: `39_cosmos_integrated_monitoring_scada_reliability_clean_blind_v7_candidate.md`
...
固定答案要素2: `PASS`。比誤差式は公式標準解答と一致する。

固定答案要素3: `PASS`。必要時の正動作と不要時の正不動作という信頼性の中核を説明している。

固定答案要素4: `FAIL / solver error`。公式標準解答は定格遮断時間を`5サイクル及び3サイクル`、事故除去指令を受ける箇所を`引外しコイル`とする。candidateは`3サイクル`だけを記載し、箇所を`引外し装置`とした。

固定答案要素5: `PASS`。投入指令と引外し指令が競合しても引外しを優先する機能と目的を説明している。
...
candidate固定後にTopic 39 canonical EXAM_ALIGNMENTと既存QAを照合した。
...
v7の不一致は教材欠落ではない。一次20答案要素は全一致し、R2固定5群境界も維持できた。固定答案要素4の不一致は独立再解答側のsolver errorである。
...
別fresh worker/contextによるclean blind v8。candidate固定前はquestion-only intakeと公式「問題」PDFだけを使い、R2二次はintake記載の固定5群境界を維持する。candidate固定後にのみ公式標準解答・既存教材と照合する。
