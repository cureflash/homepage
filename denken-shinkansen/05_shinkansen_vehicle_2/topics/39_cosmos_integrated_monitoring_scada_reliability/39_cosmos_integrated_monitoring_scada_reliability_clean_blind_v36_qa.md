# Topic 39 clean blind v36 QA

実施日: 2026-09-22
判定: `FAIL / CLEAN_BLIND_V37_REQUIRED`

## 1. Freshness

- candidate commit: `22b38789daff20f0519d9667fa3bb515cf3a1a5f`
...
| 群4 | 引外しコイル（トリップコイル）は含むが、定格遮断時間を3サイクルだけとし5サイクルを欠く | FAIL |
...
不一致診断: `solver omission ×2 secondary groups`。教材修正理由にはしない。
...
結論: `CLEAN_BLIND V36 FAIL`。テーマ完了にはしない。

## 5. 次工程

別fresh worker/contextで `clean blind v37` を実施する。candidate固定前はquestion-only intakeと公式「問題」PDFだけを用い、R2二次は固定5群境界を維持する。`25 / 25`公式一致かつ`25 / 25`教材導出可能の場合のみ最終ゲートをPASSとする。
