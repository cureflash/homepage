...
- H30一次「機械」問8: `4 / 5 FAIL`
  - (5) candidate: `ヘ：IP`
  - 公式: `イ：DHCP`
  - 問題文はIPアドレス等を自動割当てするサーバ名を要求しているため、必要語はDHCP。candidateは配布対象のアドレス種別と配布プロトコル／サーバ名を混同した。
- H25一次「機械」問8: `4 / 5 FAIL`
  - (4) candidate: `ヲ：位相振幅変調`
  - 公式: `カ：直交振幅変調`
  - 図の方式はQAMであり、選択肢上の正式な要求語は「直交振幅変調」。candidateは一般化した非選択肢名称を採用した。
...
- 群3 小問(2): `FAIL`
  - candidateは動作すべき条件で確実に動作し、不要条件で誤動作しない性質のみ記述。
  - 公式標準解答が要求する信頼性確保の要素である定期点検・自動監視、低故障率、冗長化を欠落。
- 群4 小問(3)前半: `FAIL`
  - candidate: `3サイクル`、`引外し装置`
  - 公式: `5サイクル及び3サイクル`、`引外しコイル（トリップコイル、TC可）`
  - 定格遮断時間の列挙不足と、要求部位名称の不一致。
- 群5 小問(3)後半: `PASS`
  - 投入動作・投入指令があっても引外しを優先して開路するトリップフリーの機能と目的を記述。
...
別fresh worker/contextで `clean blind v40` を実施する。candidate固定前はquestion-only intakeと公式「問題」PDF以外のanswer-bearing資料を開かない。R2二次は固定5群境界を維持する。`25 / 25`公式一致かつ`25 / 25`教材導出可能の場合のみTopic 39最終ゲートへ進む。
