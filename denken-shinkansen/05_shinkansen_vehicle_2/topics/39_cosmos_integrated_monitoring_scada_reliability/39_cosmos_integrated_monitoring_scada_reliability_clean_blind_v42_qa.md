...
- 群4 小問(3)前半: `FAIL`
  - candidate: `3サイクル`、`引外し装置（トリップコイルを含むトリップ機構）`。
  - 公式: `5サイクル及び3サイクル`、`引外しコイル（トリップコイル、TCも可）`。
  - 遮断時間の列挙不足。事故除去指令を直接受ける箇所の名称も「引外しコイル」ではなく上位概念の「引外し装置」としたため名称精度不足。
- 群5 小問(3)後半: `PASS`
  - 投入操作中・投入指令継続中でも引外しを優先し、事故継続中の投入保持・再投入を避けるトリップフリーの機能・目的を回答。

二次計: `3 / 5 FAIL`

## 教材導出可能性QA

candidate固定後に既存 `39_cosmos_integrated_monitoring_scada_reliability_explanation_source.md` と直前v41 QAを照合した。v41以降、教材本文の変更はなく、固定25答案要素を導出する記述は維持されている。
...
別fresh worker/contextで `clean blind v43` を実施する。candidate固定前はquestion-only intakeと公式「問題」PDF以外のanswer-bearing資料を開かない。R2二次は固定5群境界を維持する。`25 / 25`公式一致かつ`25 / 25`教材導出可能の場合のみTopic 39最終ゲートへ進む。
