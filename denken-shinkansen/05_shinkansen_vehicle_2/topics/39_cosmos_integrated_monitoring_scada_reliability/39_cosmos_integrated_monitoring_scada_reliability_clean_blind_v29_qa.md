# Topic 39 clean blind v29 QA

実施日: 2026-09-22
判定: `FAIL / CLEAN_BLIND_V30_REQUIRED`

## 1. Freshness

- candidate commit: `6c9b7f89c2c40a6d6cdfdab8a77efb0a7ae1acd8`
- candidate固定前に参照したTopic 39資料: 上位仕様、系列SPEC、sanitized STATUS/HANDOFF、question-only intake、固定5問の公式「問題」PDFのみ
- v1〜v28 candidate/QA、公式標準解答、固定EXAM_ALIGNMENTの保存済み正答、Topic 39 answer-bearing教材・QA: candidate固定前 `未参照`
- freshness: `PASS`
...
| H25一次「機械」問8 | `ヨ / ル / ヘ / ヲ / ト` | `ヨ / ル / ロ / カ / ト` | 3 / 5 | FAIL |

一次合計: `18 / 20`。

H25の不一致はcandidate側のsolver error / nomenclature errorであり、教材欠落ではない。
...
| 群4 | `3サイクル`＋`引外し装置（トリップ装置）` | 公式は `5サイクル及び3サイクル`＋`引外しコイル（トリップコイル、TC）` | FAIL |
| 群5 | 引外し優先、投入指令競合時も投入状態に保持しない | 要求を満たす | PASS |

二次合計: `3 / 5`。

総合: `21 / 25 FAIL`。

不一致はcandidate側の `solver error + omission + nomenclature error`。教材修正理由にはしない。

## 3. 教材だけでの導出可能性

candidate固定後、既存Topic 39 sourceとv28 QAをreconcileした。固定25答案要素に対応する教材内容は変更されていない。
...
結論: `CLEAN_BLIND V29 FAIL`。テーマ完了にはしない。

## 5. 次工程

別fresh worker/contextで `clean blind v30` を実施する。candidate固定前はquestion-only intakeと公式「問題」PDFだけを用い、R2二次は固定5群境界を維持する。`25 / 25`公式一致かつ`25 / 25`教材導出可能の場合のみ次の最終QAへ進める。
