...
candidate `39_cosmos_integrated_monitoring_scada_reliability_clean_blind_v43_candidate.md` は commit `9bf74ad27804126295e24e109b9a90469763628d` で固定した。candidate固定前は最新main、上位仕様、系列SPEC、sanitized STATUS/HANDOFF、question-only intake、固定5問の公式「問題」PDFだけを使用し、公式標準解答、既存clean-blind candidate/QA、保存済み正答、Topic 39 answer-bearing教材・QAは開いていない。
...
別fresh worker/contextで `clean blind v44` を実施する。candidate固定前はquestion-only intakeと公式「問題」PDF以外のanswer-bearing資料を開かない。R2二次は固定5群境界を維持する。`25 / 25`公式一致かつ`25 / 25`教材導出可能の場合のみTopic 39最終ゲートへ進む。
