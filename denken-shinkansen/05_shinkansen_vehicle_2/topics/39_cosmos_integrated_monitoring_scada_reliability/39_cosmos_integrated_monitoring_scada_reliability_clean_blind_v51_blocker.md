...
- latest main確認時点: `c6c3a0950035e3a51d1f7290233199f7f3d0fa9d`
...
本worker/contextはcandidate固定前の時点で、過去run由来のTopic 39 answer-bearing情報を既に保持している。加えてreconcile中に既往clean-blindの診断履歴を参照したため、`MASTER_SPEC.md` / `EXAM_ALIGNMENT_SPEC.md` が要求する独立再解答のfreshness条件を満たさない。

この状態でv51 candidateを生成すると独立再解答ゲートを偽装するため、candidate / QAは作成しない。
...
Topic 39のanswer-bearing情報をcandidate固定前に見ていない別fresh worker/contextでclean blind v51を実施する。question-only intakeと公式「問題」PDFだけでcandidateを固定・commitした後、公式標準解答・既存教材と照合する。
