...
Topic 05 `0系主変圧器の鉄心には何が起きている？` は練習問題source＋練習PDFまで完了。
...
2. 固定範囲だけで練習問題12問と完全解説を作成。
3. 固定一次5問・16小問を練習問題へ対応付けた。
4. 正答一意性・計算を12/12再検算し、A4縦3ページの練習PDFを生成。
5. 全3ページを200 dpiでレンダリングし、クリッピング・重なり・欠落・文字化けがないことを確認して表示QA PASS。
6. 二次数合わせ、変圧器等価回路、自己／相互インダクタンス、電磁誘導、同期機特性、表皮効果、未確認実車値は追加していない。

## 次に行うこと

Topic 05の解説画像PowerPointを作成する。固定EXAM_ALIGNMENT一次5問・16小問、二次非採用判断、固定範囲は変更しない。