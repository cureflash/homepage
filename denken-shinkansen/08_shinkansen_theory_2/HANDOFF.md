...
Topic 04 `架線と車体は巨大なコンデンサなのか？` は解説本文＋3段階例題、解説PDF、練習source＋練習PDF、解説画像PowerPoint、各表示QAまで完了。完成後独立再解答以降は未着手。
...
- 固定EXAM_ALIGNMENT一次5問・22小問で必要な図解だけを可視化
- 解法チェーン `D → E → V → C → W`、平行板と誘電体・分極、二層誘電体の `D` 連続と `E` 分担、同軸円筒の `D(r)`・`E(r)`・`ln(b/a)`、固定電圧／固定電荷とエネルギーを収録
- 二次に直接対応がない判断を維持し、別論点を数合わせで追加していない
- 電気影像法、導電率・コンダクタンス、力率改善、実車容量推定は追加していない
- 全5スライドをレンダリングし、クリッピング・重なりによる可読性低下・文字化け・欠落なし
- 領域外はみ出し検査 PASS

## 次に行うこと

Topic 04の固定EXAM_ALIGNMENT一次5問・22小問を、保存済み正答を見ず教材内容だけで完成後独立再解答する。

1. R6 問1(1)〜(5)、R5 問1(1)〜(5)、R3 問1(1)〜(5)、H27 問1(1)〜(2)、H24 問5(1)〜(5)を対象とする。
2. 教材だけで公式選択・途中式・設問の言い換え対応・根拠説明までできるか確認する。
3. 教材外知識で補完した小問は正答でもFAILとする。
4. 二次は直接対応がないため数合わせで追加しない。
