# STATUS - 新幹線・電験二種「理論」

更新日: 2026-09-15

## 状態
- active_series: `08_shinkansen_theory_2`
- exam_aligned_completed_topics: 7 / 21
- current_status: `topic_08_source_progress_synced`
- last_completed_topic: `07 0系のモーターはなぜ力を出せる？`
- active_topic: `08 L0系はどうやって磁石だけで浮いて進む？`
- next_start: Topic 08の最終QAを実施する

## Topic 06 制作進捗
- [x] 制作前EXAM_ALIGNMENT
- [x] 解説本文＋3段階例題
- [x] 解説PDF＋表示QA
- [x] 練習問題source＋練習PDF
- [x] 解説画像PowerPoint
- [x] 旧EXAM_ALIGNMENT完成後独立再解答: `14 / 16 PASS / NEEDS_REVISION`
- [x] EXAM_ALIGNMENT是正: H30問2(4)(5) → H21問2(2)(3)
- [x] 修正版EXAM_ALIGNMENT完成後独立再解答: `16 / 16 PASS`
- [x] 練習sourceの過去問対応表をH21選定へ同期
- [x] 主sourceの進捗記録を現在地へ同期
- [x] 最終QA再々実施: `PASS / completed`

## Topic 07 制作進捗
- [x] 制作前EXAM_ALIGNMENT: 一次5問・25小問
- [x] R7〜R5二次を確認し、固定範囲の直接対応問題なし
- [x] 二次数合わせ 0件
- [x] 解説本文＋3段階例題
- [x] 解説PDF＋表示QA
- [x] 練習問題source＋練習PDF
- [x] 解説画像PowerPoint
- [x] 完成後独立再解答: `25 / 25 PASS`
- [x] 主source・練習sourceの進捗記録同期
- [x] 最終QA再実施: `PASS / completed`

## Topic 08 固定EXAM_ALIGNMENT
- [x] MASTER_SPEC / EXAM_ALIGNMENT_SPEC / 系列SPEC確認
- [x] 固定範囲: ファラデーの法則、レンツの法則、誘導電流、渦電流、電磁力、磁気エネルギー
- [x] R7 一次 理論 問2(2): 結合コイル系の磁気エネルギー
- [x] R5 一次 理論 問2(2)〜(4): 磁束変化、速度、ファラデー則
- [x] R2 一次 理論 問2(2): `W=LI²/2`
- [x] H28 一次 理論 問1(5): `W=LI²/2`
- [x] 品質ゲート対象を一次4問・6小問に固定
- [x] 公式解答照合 `6 / 6`
- [x] 二次の直接対応問題なし、二次数合わせ0件
- [x] 固定範囲外論点・超電導物性追加0件

## Topic 08 制作進捗
- [x] 制作前EXAM_ALIGNMENT
- [x] 解説本文
- [x] 3段階例題
- [x] 解説PDF＋表示QA
- [x] 練習問題source＋練習PDF
- [x] 解説画像PowerPoint
- [x] 完成後独立再解答: `6 / 6 PASS`
- [x] 主sourceの進捗記録同期
- [ ] 最終QA

## 今回進めた内容

Topic 08主source `08_maglev_electromagnetic_induction.md` の旧進捗記録2箇所を実成果物へ同期した。

- 冒頭の状態欄を、解説PDF・練習source/PDF・PowerPoint・完成後独立再解答まで完了した現在地へ更新
- 末尾 `## 次工程` を最終QAへ更新
- 技術本文、固定EXAM_ALIGNMENT、数式、3段階例題、問題、PDF/PPTX、独立再解答結果は変更していない
- 独立再解答 `6 / 6 PASS`、教材外知識補完0件を維持
- 二次採用0問・二次数合わせ0件を維持
- 固定範囲外論点・超電導物性・未確認実設備値追加0件

## 進捗記録不整合

主sourceの既知の旧進捗2箇所は解消済み。

## 判定

Topic 08は `SOURCE_PROGRESS_SYNCED / IN_PROGRESS`。完成数は `7 / 21` のまま。次工程は最終QA。