...
状態: `topic_02_practice_qa_complete`
...
練習問題は正式選定6問の要求事項を問題マッピングで全てカバーした。公式過去問の転載ではなく、同型問題へ転用できるか確認する独自問題である。

独立QAでは、一次型10問の正答肢が各問1つだけ成立することを確認した。計算問題は `P=9.8QHη` と水量収支から再計算し、数値・単位・丸めが保存済み完全解説と一致した。二次型5問は途中式または記述採点要点が採点可能な形で整合している。
...
Topic 02 `信濃川の水で新幹線を何本走らせられる？` の解説PDFを作成する。

- `02_hydropower.md` の本文・3段階例題・EXAM_ALIGNMENTを同期する。
- sourceにある出典・数式・単位を落とさない。
- 信濃川水系の実在値は一次資料確認済みの値だけを実値として扱う。
- 列車1本の所要電力には未確認の仮定値を追加しない。
- 固定範囲外の論点は追加しない。