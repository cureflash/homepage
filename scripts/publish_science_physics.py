...
try:
    ...
    from science_physics_atomic import PHYSICS_ATOMIC_PROBLEM_COUNT, PHYSICS_ATOMIC_TOPICS
...
ALL_TOPICS = {
    ...
    **{key: (topic, PHYSICS_ATOMIC_PROBLEM_COUNT) for key, topic in PHYSICS_ATOMIC_TOPICS.items()},
}
    ...
    for topic_key, (topic, problem_count) in ALL_TOPICS.items():
        for mode_key, mode in topic["modes"].items():
            for variant, seed in enumerate(topic["seeds"], start=1):
                ...
                entry = {"id": wid, "school_level": "high-school", "grade": None, "formal_course": FORMAL_COURSE, "subject": "理科", "science_field": "physics", "worksheet_mode": mode["worksheet_mode"], "unit": topic["unit"], "skill": topic["skill"], "problem_count": len(problems), "seed": seed, "variant": variant, "title": title, "description": mode["description"], "url": url, "content_hash": content_hash, "difficulty": "basic", "worksheet_series": "focused", "answer_type": answer_type}
                if wid in by_id:
                    existing = by_id[wid]
                    ...
                    continue
                if url in by_url: raise AssertionError(f"worksheet URL already belongs to another id: {url}")
                if content_hash in by_hash or any(item["entry"]["content_hash"] == content_hash for item in pending): raise AssertionError(f"duplicate worksheet content: {wid}")
...
def publish(repo_root):
    ...
    with tempfile.TemporaryDirectory(dir=root) as tmp:
        ...
        for item in pending:
            ...
            if not rendered.is_file() or rendered.stat().st_size <= 1000: raise AssertionError(f"PDF rendering failed: {item['filename']}")
        for item in pending:
            destination = output_dir / item["filename"]
            if destination.exists(): raise AssertionError(f"refusing to overwrite: {destination}")
