...
def build_batch(repo_root):
    ...
    existing_urls = {entry["url"] for entry in catalog}
    ...
    for topic_key, topic in JH1_PHYSICS_FORMULA_TOPICS.items():
        for mode_key, mode in topic["modes"].items():
            for variant, seed in enumerate(JH1_PHYSICS_SEEDS, start=1):
                ...
                if wid in existing_ids or url in existing_urls:
                    raise AssertionError(f"worksheet id/url already exists: {wid}")
                ...
                pending.append({"entry": entry, "problems": problems, "filename": filename, "title": topic["title"]})
...
def publish(repo_root):
    ...
    print(f"published {len(pending)} junior-high grade-1 physics worksheets")
