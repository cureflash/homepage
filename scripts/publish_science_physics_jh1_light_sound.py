...
try:
    ...
    from science_physics_jh1_light_sound_topics import (
        JH1_LIGHT_SOUND_PROBLEM_COUNT,
        JH1_LIGHT_SOUND_SEEDS,
        JH1_PHYSICS_LIGHT_SOUND_TOPICS,
    )
        ...
        for mode_key, mode in topic["modes"].items():
            for variant, seed in enumerate(JH1_LIGHT_SOUND_SEEDS, start=1):
                ...
                entry = {
                    ...
                    "unit": topic["unit"],
                    "skill": topic["skill"],
                    "problem_count": JH1_LIGHT_SOUND_PROBLEM_COUNT,
                    "seed": seed,
                    "variant": variant,
                    "title": title,
...
def publish(repo_root):
    ...
    if not pending:
        ...
        print("junior-high grade-1 light/sound worksheets already published")
        return 0
    ...
    print(f"published {len(pending)} junior-high grade-1 light/sound worksheets")
    return len(pending)
